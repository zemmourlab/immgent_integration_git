...
out_dir = "~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20260107" 
out_dir = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20260107"
...
--> in TF_analysis_20251128.Rmd > /project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260107
igt1_96_pseudobulk_byclusterannotationlevel1_log1pCP10K.Rds
igt1_96_pseudobulk_byclusterannotationlevel2_log1pCP10K.Rds
...
```{r}
pseudobulk_orig = readRDS(sprintf("%s/igt1_96_pseudobulk_byclusterannotationlevel1_log1pCP10K.Rds", "/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260107"))
...
**Gene_AUC function**
--> in TF_analysis_20251128.Rmd
modify the function to add in the heatmap annotation for columns based on pseudobulk@meta.data columns annotation_level1 and annotation_level2_group
functino options provide the columns in pseudobulk@meta.data to use
and a list of color palette to use for each column (list("annotation_level1" = mypal_level1, "annotation_level2_group" = c(resting = "blue", activated = "red", other = "black")))
...
out_dir2 = sprintf("%s/%s", out_dir, "level1b")
...
**Across Level2**
```{r}
pseudobulk_orig = readRDS(sprintf("%s/igt1_96_pseudobulk_byclusterannotationlevel2_log1pCP10K.Rds", "/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260107"))
pseudobulk_orig$annotation_level2 = gsub("\\-", "_", pseudobulk_orig$annotation_level2)
pseudobulk_orig$annotation_level1 = sub("_.*", "", pseudobulk_orig$annotation_level2)
pseudobulk_orig$annotation_level2_group = so_orig$annotation_level2_group[match(pseudobulk$annotation_level2,so_orig$annotation_level2)] %>% as.character()
...
pseudobulk = pseudobulk_orig
out_dir2 = sprintf("%s/%s", out_dir, "level2")
Gene_AUC(pseudobulk = pseudobulk,
    genes = effmol, # vector of genes
    out_dir = out_dir2,
    mypal = mypal,              # named vector of colors for TF lines
    ...
    pdf_width = 12, #15 x 15 for large heatmap, 12 x 5 for barplots
    pdf_height = 5
)
...
# out_dir2 = sprintf("%s/%s", out_dir, "level2Top50")
# Gene_AUC(pseudobulk,
#          genes = effmol, # vector of genes
#          out_dir = out_dir2,
#          mypal = mypal,              # named vector of colors for TF lines
...
curated_list = read.table('/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20260107/effmol_curated.txt', header = F, skip = 0, sep = "\t")[,1]
out_dir2 = sprintf("%s/%s", out_dir, "level2CuratedListNoDP")
Gene_AUC(pseudobulk[,pseudobulk$annotation_level1 != "DP"],
         genes = curated_list, # vector of genes
         out_dir = out_dir2,
         mypal = mypal,              # named vector of colors for TF lines
         order_groups = levels(so_orig$annotation_level2),
...
curated_list = read.table('/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20260107/effmol_curated.txt', header = F, skip = 0, sep = "\t")[,1]
out_dir2 = sprintf("%s/%s", out_dir, "level2CuratedListNoThreshold")
Gene_AUC(pseudobulk,
         genes = curated_list, # vector of genes
         out_dir = out_dir2,
         mypal = mypal,              # named vector of colors for TF lines
         ...
         mypal_groups = mypal_level1b,       # named vector of colors for groups in barplots
         n_top = 50,                # number of top TF to highlight
         expr_threshold = 0,       # threshold for filtering genes, in CK10K
         cols_heatmap_gradient = colorRampPalette(c('white','red','brown'))(100), #rev(colorRampPalette(brewer.pal(n = 7,name = "RdYlBu"))(100))  rev(viridis::rocket(100))
         ann_cols = ann_cols,
         ann_palettes = ann_palettes,
         pdf_width = 15,
...
curated_list = read.table('/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20260107/effmol_curated.txt', header = F, skip = 0, sep = "\t")[,1]
out_dir2 = sprintf("%s/%s", out_dir, "level2CuratedListNoThresholdNoDP")
Gene_AUC(pseudobulk[,pseudobulk$annotation_level1 != "DP"],
         genes = curated_list, # vector of genes
         out_dir = out_dir2,
         mypal = mypal,              # named vector of colors for TF lines
...
for (ct in c("CD8", "CD4", "Treg", "gdT", "CD8aa", "Tz", "DN", "DP")) {
    print(ct)
    out_dir2 = out_dir2 = sprintf("%s/%s", out_dir, ct)
...
OUT_DIR IS 20251218 HERE! "~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20251218"
...
pdf(sprintf("%s/Barplot_Pseudobulk_Foxp3_Il4_Ifng_Il17a_S100a8.pdf",out_dir), 10,5, useDingbats = F)
...
stick to varianceFano doesn't seem much better, maybe CV^2, 
```{r}
library(dplyr)
library(ggplot2)
library(Matrix)
...
genelist = read.table('/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/EffMol_analysis/20260107/level2CuratedList/AUC.txt', header = T)
...
# pdf(sprintf("%s/ScatterPlot_VarDecomp_ClusterSample2.pdf",out_dir), 10,10, useDingbats = F)
pdf(sprintf("%s/ScatterPlot_VarDecomp_ClusterSample2.pdf",out_dir), 10,10, useDingbats = F)
