# David Zemmour
# R
# usage: Rscript limma_fit_20260320_level2program.IGTHT.R [path_to_seurat_object] [path_to_tmm_object] [output_dir] [fit_file_name]
...
if (length(args) < 4) {
    stop("Usage: Rscript limma_fit_20260320_level2program.IGTHT.R [path_to_seurat_object] [path_to_tmm_object] [output_dir] [fit_file_name]")
}
...
message("Fitting")
design = data.frame(so@meta.data[,c("annotation_level2_program","IGTHT")]) 
design$annotation_level2_program.IGTHT = paste(design$annotation_level2_program, design$IGTHT, sep = ".")
...
design = model.matrix(~ 0 + annotation_level2_program.IGTHT, data=design) 
colnames(design) = gsub("annotation_level2_program.IGTHT", "", colnames(design))
