...
**Palette for nonoverlapping colors for all: mypal_level2_all** 
```{r}
library("pals")
n = 70
...
mypal_list[["level2_all"]] = setNames(mypal, unique(so_orig$annotation_level2))
mypal_list[["level2_all"]]  = mypal_list[["level2_all"]] [!is.na(names(mypal_list[["level2_all"]] ))]

# DimPlot(so_orig, reduction = "mde2_totalvi_20241006", group.by = "annotation_level2", raster = T, cols = mypal_list[["level2_all"]]) + NoLegend()
...
cls = c("blue","darkorange","darkolivegreen","darkorchid",
        ...
        "cadetblue","brown","darkseagreen","violetred",
        "cyan","burlywood", "red")
mypal = c(sprintf("%s1",cls),sprintf("%s4",cls))
# mypal = c(polychrome())
names(mypal) = NULL

mypal_list[["organ_simplified"]] = setNames(mypal, unique(so_orig$organ_simplified))
mypal_list[["organ_simplified"]]  = mypal_list[["organ_simplified"]] [!is.na(names(mypal_list[["organ_simplified"]] ))]

DimPlot(so_orig, reduction = "mde2_totalvi_20241006", group.by = "organ_simplified", raster = T, cols = mypal_list[["organ_simplified"]], split.by = "organ_simplified", ncol = 5) + theme_dark()
...
**Print palettes**
```{r}
ensure_directory("immgent_color_palettes")

pdf("immgent_color_palettes/MDE_color_palette.pdf", 10,10, useDingbats = F)
...
p = DimPlot(so_orig, reduction = "mde2_totalvi_20241006", group.by = "annotation_level2", raster = T, cols = mypal_list[["level2_all"]]) + 
    ggtitle(sprintf("mypal_list[['%s']]","level2_all")) #+ NoLegend()
print(p)
print(p + NoLegend())

dev.off()

pdf("immgent_color_palettes/MDE_color_palette_organ_simplified.pdf", 15,15, useDingbats = F)
...
saveRDS(mypal_list, "immgent_color_palettes/immgent_color_palettes_list_20250708.Rds")

for (i in names(mypal_list)) {
    write.table(mypal_list[[i]],file = sprintf("immgent_color_palettes/immgent_color_palettes_%s.csv",i),quote = F, sep = ",", col.names = F)
}
