...
libs = c("ZemmourLib","Seurat","ggplot2", "dplyr", "reshape2", "RColorBrewer", "pals", "scales") 
...
so_orig = readRDS("/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107_clean_ADTonly.Rds")
so = list()
so[["CD4"]] = so_orig[,so_orig$annotation_level1 == "CD4"]
so[["CD8"]] =  so_orig[,so_orig$annotation_level1 == "CD8"]
so[["Treg"]] =  so_orig[,so_orig$annotation_level1 == "Treg"]
so[["gdT"]] = so_orig[,so_orig$annotation_level1 == "gdT"]
so[["CD8aa"]] = so_orig[,so_orig$annotation_level1 == "CD8aa"]
so[["Tz"]] = so_orig[,so_orig$annotation_level1 == "Tz"]
so[["DN"]] = so_orig[,so_orig$annotation_level1 == "DN"]
so[["DP"]] = so_orig[,so_orig$annotation_level1 == "DP"]
...
mypal_list[["level1"]] = c(CD4 = "blue", CD8 = "darkorange2", Treg = "deeppink", gdT = "chartreuse3", CD8aa = "darkorchid", Tz = "darkgoldenrod1",DN = "deepskyblue",DP="red",thymocyte = "grey")
...
mypal_list[["level2"]] = c(mypal_list[["CD4"]], mypal_list[["CD8"]], mypal_list[["Treg"]], mypal_list[["gdT"]], mypal_list[["Tz"]], mypal_list[["CD8aa"]], mypal_list[["DN"]], mypal_list[["DP"]],mypal_list[["thymocyte"]],mypal_list[["nonT"]], mypal_list[["remove"]] , mypal_list[["unclear"]] )
...
**Palette for Topics**
```{r}
cls = colors()[!grepl("gray|grey",colors()) ]
mypal_topics = sample(cls, 200)
# mypal_topics = RColorBrewer::brewer.pal(8,name = "Accent")
names(mypal_topics) = paste("F", 1:200, sep = "")
mypal_topics["F1"] = "blue"
mypal_topics["F68"] = "deeppink"

mypal_list[["topics"]] = mypal_topics

sample_df <- data.frame(
  category = factor(names(mypal_topics), levels = names(mypal_topics)),
  value = 1)

pdf(sprintf("%s/Topic_color_palette_organ_simplified.pdf", prefix), 60,10, useDingbats = F)
ggplot(sample_df, aes(x = category, y = value, fill = category)) +
  geom_col() +
  scale_fill_manual(values = mypal_topics) +
  theme_minimal() +
  labs(title = "mypal_topics Palette") + NoLegend() + NoGrid()
dev.off()
...
# mypal_list = readRDS(sprintf("%s/immgent_color_palettes_list_%s.Rds", prefix, prefix))
...
**Update palettes with new annotation prefix = 20260206 **

```{r}
prefix = "20260206"

mypal_list = readRDS(sprintf("%s/immgent_color_palettes_list_%s.Rds", "20260109", "20260109"))
mypal_list_orig = mypal_list
mypal_list_orig[["level2"]]

# mypal_list2 = readRDS(sprintf("%s/immgent_color_palettes_list_%s.Rds", "20250714", "20250714"))
# mypal_list_orig2 = mypal_list2
# mypal_list_orig2[["level2"]]
# mypal_list_orig[["level2"]][grepl("CD4", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("CD4", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][grepl("CD8", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("CD8", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][grepl("CD8aa", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("CD8aa", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][grepl("gdT", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("gdT", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][grepl("Tz", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("nonconv", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][grepl("DN", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("DN", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][grepl("Treg", names(mypal_list_orig[["level2"]]) )] == mypal_list_orig2[["level2"]][grepl("Treg", names(mypal_list_orig2[["level2"]]) )]
# mypal_list_orig[["level2"]][mypal_list_orig[["level2"]] == "Treg_cl8"] = "chartreuse2"
...
names(mypal_list[["level2"]]) = key$new[match(names(mypal_list[["level2"]]), key$old)]
mypal_list[["level2"]] = mypal_list[["level2"]][na.omit(names(mypal_list[["level2"]]))]
mypal_list[["level2"]]  = mypal_list[["level2"]][unique(names(mypal_list[["level2"]]))]
mypal_list[["level2"]][grepl("\\.P", names(mypal_list[["level2"]]))] = "black"
mypal_list[["level2"]][grepl("\\.wM", names(mypal_list[["level2"]]))] = "grey"
mypal_list[["level2"]][names(mypal_list[["level2"]]) == "Treg.E"] = "chartreuse2"
mypal_list[["level2"]][names(mypal_list[["level2"]]) == "CD4.H"] = "darkseagreen2"
mypal_list[["level2"]][names(mypal_list[["level2"]])== "CD4.Q"] = "darkorange"
mypal_list[["level2"]][names(mypal_list[["level2"]])== "Tz.A"] = "darkorchid2"
mypal_list[["level2"]][names(mypal_list[["level2"]])== "Tz.B"] = "deepskyblue2"

mypal_list[["CD8"]] = NULL
mypal_list[["CD4"]] = NULL
mypal_list[["Treg"]] = NULL
mypal_list[["gdT"]] = NULL
mypal_list[["Tz"]] = NULL
mypal_list[["CD8aa"]] = NULL
mypal_list[["DN"]] = NULL
mypal_list[["DP"]] = NULL
mypal_list[["thymocyte"]] = NULL
mypal_list[["nonT"]] = NULL
mypal_list[["remove"]] = NULL
mypal_list[["unclear"]] = NULL

names(mypal_list[["level2_option2"]]) = key$new[match(names(mypal_list[["level2_option2"]]), key$old)]
mypal_list[["level2_option2"]][grepl("\\.P", names(mypal_list[["level2_option2"]]))] = "black"
mypal_list[["level2_option2"]][grepl("\\.wM", names(mypal_list[["level2_option2"]]))] = "grey"
...
which(so_orig[["mde_incremental"]]@cell.embeddings[so_orig$annotation_level2 %in% "CD4_cl13",1] < 1 & so_orig[["mde_incremental"]]@cell.embeddings[so_orig$annotation_level2 %in% "CD4_cl13",2] < -1.5) %>% length()
...
pal_anno <- unique(so_orig$annotation_level2) %>% 
  setNames(.,.) %>% 
  sapply(get_clkey) %>%                # turn Treg_cl1 → "cl1", etc
