...
Continuation from clustering_CD8AB_igt_1_96.Rmd
...
Remove IGT90: CD8AB5.1/2 not the panel
remove protein with less than 1000 rowSums, except TCRVA11.1-TCRVA11.2
--> cometsc_20240607/totalvi_igt1_56_allgenes_CD8AB_20240529_so_withclusters_filteredforCometsc.Rds
```{r}
so = readRDS("igt1_96_CD8AB_20241017.Rds")
library(reshape2)
library(pheatmap)
so_orig = so
...
saveRDS(so, sprintf("%s/igt1_96_CD8AB_filteredforCometsc.Rds", prefix))

```

Heatmap Clusters ADT
```{r}
so = readRDS(sprintf("%s/igt1_96_CD8AB_filteredforCometsc.Rds", prefix))

so = NormalizeData(so, assay = "ADT", normalization.method = "CLR")
adt = so[["ADT"]]$data
tmp = as.data.frame(t(as.matrix(adt)))
tmp$CD8AB_clusters = so$CD8AB_clusters
tmp2 = melt(tmp)
tmp2 = tmp2 %>% group_by(CD8AB_clusters, variable) %>% summarise(mean(value))
tmp2 = dcast(tmp2, variable ~ CD8AB_clusters)
rownames(tmp2) = tmp2[,1]
tmp2 = tmp2[,-1]
#tmp2 = apply(tmp2,1, as.numeric)
pdf(sprintf("%s/heatmap_ADT_clusters.pdf", prefix), 10,15)
pheatmap(log10(tmp2+1), cluster_rows = F, cluster_cols = F, main = "Mean CLR ADT per CD8AB_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = T, main = "Mean CLR ADT per CD8AB_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = F, main = "Mean CLR ADT per CD8AB_clusters")
dev.off()
...
```{bash}
cd /project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD8AB/cometsc_20241023
...
so = NormalizeData(so, assay = "ADT", normalization.method = "LogNormalize")
adt = as.matrix(so[["ADT"]]$data)
markers = c("CD62L", "KLRG1", "CD8AB9B", "FR4", "SCA1", "NEUROPILIN1.CD304", "CD69", "CD8AB5RB")
adt = adt[markers,]
tmp = melt(adt)
...
# pdf("markers_CD8ABclusters_20240612_gatingplots.pdf", 5, 5, useDingbats = F)
# pdf("markers_CD8ABclusters_20240612_gatingplots_organ.pdf", 15, 15, useDingbats = F)
...
#cl3 no pregating, just on CD8AB9B pos
cl = "cl3"
feature1 = "CD62L"
feature2 = "CD8AB9B"
th1 = 5
...
for (i in unique(gates$organ_simplified)){
    ...
    # write.table(stats, file = make.names(sprintf("markers_CD8ABclusters_20240612_statsSeSp_%s.tsv",i)), quote = F, row.names = T, col.names = T, sep = "\t")
