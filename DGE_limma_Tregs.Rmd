...
# tcr_genes = rownames(so)[grepl(x = rownames(so), pattern = "Trbv|Trbd|Trbj|Trbc|Trav|Traj|Trac|Trbv|Trbd|Trbj|Trbc|Trdv|Trdj|Trdc")]
...
totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_id2.Rds: 0 + Cluster_totalvi20240525rmigtsample_Res0.5.sample_id
...
should be equivalent to analyze the data.
```{r}
...
design = model.matrix(~ 0 + Cluster_totalvi20240525rmigtsample_Res0.5.sample_id, data=design) #totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_id2.Rds
...
# fit = lmFit(tmm, design = design)
# saveRDS(fit, file = "totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_id2.Rds")

fit = readRDS("totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5.sample_id.Rds")

```

QUESTION 3: Each Tregs Cluster_totalvi_20240525_rmigtsample_Res0.5 vs all others, only in healthy tissues, independently of sample_id
```{r}
...
```{r}
table(so$sample_type, so$batch)
group = as.character(so$ClusterCCA_Revised)
group[!so$sample_type %in% c("WT.CD4")] = "notWT"
group = factor(group)
x = levels(group)
x = x[x != "notWT"]

contrasts = sapply(x, FUN = function(i) sprintf("%s-(%s)/%s", i, paste(x[x!= i], collapse = "+"), length(x)-1))
namescontrasts = paste(x, "_vs_OtherClustersInWT", sep = "")
names(contrasts) = namescontrasts
contrasts
gene_symbol = rownames(so@assays$RNA@counts)
confoundings = list(so$sample_name, paste("exp", so$batch, sep = "_"))
formula.mod.matrix = "~0+group+confoundings[[1]]+confoundings[[2]]"
...
QUESTION 1: Each Tregs Cluster_totalvi_20240525_rmigtsample_Res0.5 vs all others independently of sample_id
```{r}
group = paste(so$Cluster_totalvi20240525rmigtsample_Res0.5)
...
contrasts = sapply(x, FUN = function(i) sprintf("%s-(%s)/%s", i, paste(x[x!= i], collapse = "+"), length(x)-1))
namescontrasts = paste(x, "_vs_OtherClusters", sep = "")
names(contrasts) = namescontrasts
contrasts
gene_symbol = rownames(so[["RNA"]]$counts)
...
pdf(sprintf("%s_DiffExpression_volcano1.pdf", prefix), width = 10, height = 10)
for (i in 1:length(tmp)) {
  print(i)
  vplot = data.frame(SYMBOL = tmp[[i]]$SYMBOL, fc = 2^tmp[[i]]$logFC, pval = tmp[[i]]$P.Value+10^-300, AveExpr = 2^(tmp[[i]]$AveExpr))
  p = Vplot(vplot = vplot, xlab = names(tmp)[i], xlimits = c(min(vplot$fc[vplot$fc != 0]), max(vplot$fc)), ylimits = c(min(vplot$pval), 1)) #c(min(vplot$pval[vplot$pval != 0]),1)
...
QUESTION 2: Each Tregs Cluster_totalvi_20240525_rmigtsample_Res0.5 vs all others, only in healthy tissues, independently of sample_id
```{r}
group = paste(so$Cluster_totalvi20240525rmigtsample_Res0.5)
group[!so$condition_broad %in% c("healthy")] = "notincluded"
unique(group)
...
x = levels(group)
x = x[x != "notincluded"]
table(group, so$condition_broad)
#table(group, so$sample_name)
#groups  = unique(gsub("_rep.*$", "", x))
cat(x,sep = "\",\"")

contrasts = sapply(x, FUN = function(i) sprintf("%s-(%s)/%s", i, paste(x[x!= i], collapse = "+"), length(x)-1))
namescontrasts = paste(x, "_vs_OtherClustersinHealthy", sep = "")
names(contrasts) = namescontrasts
contrasts
gene_symbol = rownames(so[["RNA"]]$counts)
confoundings = list(so$sample_id)
...
contrasts
gene_symbol = rownames(so[["RNA"]]$counts)
confoundings = list(so$organ_simplified,so$sample_id)
formula.mod.matrix = "~0+group+confoundings[[1]]+group*confoundings[[1]]+confoundings[[2]]"
...
```{r}
tt_list = readRDS("ttlist_20240603.Rds")
tt_list = c(tt_list_1, tt_list_2)
saveRDS(tt_list, file = "ttlist_20240603.Rds")
```
...
ttl$log2FC["Tbx21",ttl$adj.P.Val["Tbx21",] < 0.05]
ttl$adj.P.Val["Tbx21",ttl$adj.P.Val["Tbx21",] < 0.05]
