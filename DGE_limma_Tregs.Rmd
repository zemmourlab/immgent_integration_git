...
**Functions** --> also in custom_functions_david.R
```{r}
...
prefix = "DGE_limma"
prefix2 = "Cluster_totalvi20240525rmigtsample_Res0.5Sample_idConcat"
```
...
```{r}
tmm = readRDS(sprintf("%s/totalvi_igt1_56_allgenes_Treg_20240529_tmm.Rds",prefix))
...
# genes_to_keep = colSums(counts) >0 & !(rownames(so) %in% c(tcr_genes, gm_rik_genes, ribo_genes, mt_genes))
...
saveRDS(tmm, file = sprintf("%s/totalvi_igt1_56_allgenes_Treg_20240529_tmm.Rds", prefix))

```

**Building the design matrix and model**
totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_id.Rds: 0 + Cluster_totalvi20240525rmigtsample_Res0.5 + sample_id 
totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_idConcat.Rds: 0 + Cluster_totalvi20240525rmigtsample_Res0.5.sample_id
totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_idWithInteraction.Rds: 0 + Cluster_totalvi20240525rmigtsample_Res0.5 + sample_id + Cluster_totalvi20240525rmigtsample_Res0.5:sample_id
totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5ConditionDetailedOrgansimnplifiedSexIGT.Rds: 0 + Cluster_totalvi20240525rmigtsample_Res0.5 + condition_detailed + organ_simplified + sex + IGT


~ Cluster_totalvi20240525rmigtsample_Res0.5 + condition_detailed + organ_simplified + sex + IGT: 
intercept here is cl1 spleen baseline male IGT1
It defines effect of each variable vs the intercept. 
cl2-cl3 is same effect regardless of tissue, IGT, sex, condition_detailed. And this effect is Cl2 and Cl3 holding all other factors at their reference levels (spleen baseline male IGT1).
we can't calculate cl2inColon-cl3inColon specifically without adding an interactino in the model.matrix Cluster_totalvi20240525rmigtsample_Res0.5:organ_simplified
intercept-cl2, calculate intercept (cl1 spleen baseline male IGT1) - cl2 (in spleen baseline male IGT1)
if you decide to include interaction terms to see how the difference between Cl2 and Cl3 varies with different conditions, organs, etc., you would modify your model like this:
design <- model.matrix(~ Cluster_totalvi20240525rmigtsample_Res0.5 * (condition_detailed + organ_simplified + sex + IGT), data=design) but that also calculate the interactions in their reference level.

So I am not sure this works well to interpret our data. 
Same problems arise with Cluster_totalvi20240525rmigtsample_Res0.5 + sample_id 
A possible solution would be to use no intercept, but it doesn't work when more than 1 categories and it makes computation very expensive

New strategy: ~ 0 + Cluster_totalvi20240525rmigtsample_Res0.5.sample_id: linear model with cluster and variable. Each coefficient calculated for Cluster_totalvi20240525rmigtsample_Res0.5.sample_id
And combine Cluster_totalvi20240525rmigtsample_Res0.5.sample_id as needed to make the comparisons.
-->totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_idConcat.Rds
```{r}
design = data.frame(so@meta.data[,c("Cluster_totalvi20240525rmigtsample_Res0.5","sample_id")]) 
design$Cluster_totalvi20240525rmigtsample_Res0.5.sample_id = paste(design$Cluster_totalvi20240525rmigtsample_Res0.5, design$sample_id, sep = ".")
all(rownames(design) == colnames(so))
design = model.matrix(~ 0 + Cluster_totalvi20240525rmigtsample_Res0.5.sample_id, data=design) #totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_idConcat.Rds
colnames(design) = gsub("Cluster_totalvi20240525rmigtsample_Res0.5.sample_id", "", colnames(design))
...
(Other trials discussed)
```{r}
so$organ_simplified.orig = so$organ_simplified
so$condition_detailed.orig = so$condition_detailed
so$sex.orig = so$sex
so$organ_simplified = make.names(so$organ_simplified) %>% as.factor()
so$organ_simplified = relevel(so$organ_simplified, ref = "spleen")
so$condition_detailed = make.names(so$condition_detailed) %>% as.factor()
so$condition_detailed = relevel(so$condition_detailed, ref = "baseline")
so$sex = make.names(so$sex) %>% as.factor()
so$sex = relevel(so$sex, ref = "male")
so$Cluster_totalvi20240525rmigtsample_Res0.5 = relevel(so$Cluster_totalvi20240525rmigtsample_Res0.5, ref = "cl1")

design = data.frame(so@meta.data[,c("Cluster_totalvi20240525rmigtsample_Res0.5","sample_id")]) #c("Cluster_totalvi20240525rmigtsample_Res0.5","sample_id", "organ_simplified", "condition_detailed", "sex")
design$Cluster_totalvi20240525rmigtsample_Res0.5.sample_id = paste(design$Cluster_totalvi20240525rmigtsample_Res0.5, design$sample_id, sep = ".")
all(rownames(design) == colnames(so))

design = model.matrix(~ 0 + Cluster_totalvi20240525rmigtsample_Res0.5 + sample_id , data=design) #totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_id.Rds
colnames(design) = gsub(paste(c("Cluster_totalvi20240525rmigtsample_Res0.5","sample_id"), collapse = "|"), "", colnames(design))

design = model.matrix(~ 0 + Cluster_totalvi20240525rmigtsample_Res0.5 + sample_id + Cluster_totalvi20240525rmigtsample_Res0.5:sample_id, data=design) 
colnames(design) = gsub(paste(c("Cluster_totalvi20240525rmigtsample_Res0.5","sample_id"), collapse = "|"), "", colnames(design)) colnames(design)#totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_idWithInteraction.Rds
#colnames(design) = gsub(paste(c("Cluster_totalvi20240525rmigtsample_Res0.5","sample_id"), collapse = "|"), "", colnames(design))

design = data.frame(so@meta.data[,c("Cluster_totalvi20240525rmigtsample_Res0.5","organ_simplified", "condition_detailed", "sex", "IGT")])
design = model.matrix(~ 0+Cluster_totalvi20240525rmigtsample_Res0.5 + condition_detailed + organ_simplified + sex + IGT, data=design) 
#colnames(design) = gsub(paste(c("Cluster_totalvi20240525rmigtsample_Res0.5","condition_detailed", "organ_simplified","sex"), collapse = "|"), "", colnames(design))
#colnames(design) = gsub("IGTIGT", "IGT", colnames(design))
colnames(design)
fit = lmFit(tmm, design = design)
#Coefficients not estimable: B16.tumor mammary.gland synovial.fluid IGT24 IGT29 IGT43 IGT52

design = data.frame(so@meta.data[,c("Cluster_totalvi20240525rmigtsample_Res0.5","organ_simplified", "condition_detailed", "sex", "IGT")])
design = model.matrix(~ Cluster_totalvi20240525rmigtsample_Res0.5 + condition_detailed + organ_simplified + sex + IGT, data=design) 
#colnames(design) = gsub(paste(c("Cluster_totalvi20240525rmigtsample_Res0.5","condition_detailed", "organ_simplified","sex"), collapse = "|"), "", colnames(design))
#colnames(design) = gsub("IGTIGT", "IGT", colnames(design))
colnames(design)
...
# fit = lmFit(tmm, design = design)
# saveRDS(fit, file = "totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5Sample_idConcat.Rds")

fit = readRDS("totalvi_igt1_56_allgenes_Treg_20240529_fit_onCluster_totalvi20240525rmigtsample_Res0.5ConditionDetailedOrgansimnplifiedSexIGT.Rds")
design = fit$design
colnames(design)
```

Each cl vs all the others in Healthy
```{r}
...
for (cl in levels(metadata$Cluster_totalvi20240525rmigtsample_Res0.5)) {
    group1_filter <- expr(Cluster_totalvi20240525rmigtsample_Res0.5 == cl & condition_broad == "healthy")
    group2_filter <- expr(!(Cluster_totalvi20240525rmigtsample_Res0.5 %in% cl) & condition_broad == "healthy")
    groups = GetGroups(metadata, group1_filter,group2_filter, "Cluster_totalvi20240525rmigtsample_Res0.5.sample_id")
    nmecontrast = sprintf("%s_vs_AllOthersInHealthy", cl)
...
group1 = metadata %>% filter(Cluster_totalvi20240525rmigtsample_Res0.5 == "cl1" & condition_broad == "healthy") # &\ %>% pull(Cluster_totalvi20240525rmigtsample_Res0.5.sample_id)
group2 = metadata %>% filter(Cluster_totalvi20240525rmigtsample_Res0.5 == "cl1" & condition_broad == "virus") #%>% pull(Cluster_totalvi20240525rmigtsample_Res0.5.sample_id)
group1$group = "group1"
group2$group = "group2"
combined_groups = rbind(group1, group2)
results = lapply(colnames(metadata)[!colnames(metadata) %in% c("sample_id","Cluster_totalvi20240525rmigtsample_Res0.5.sample_id")], test_balanced)  %>% unlist() %>% suppressWarnings()
names(results) = colnames(metadata)[!colnames(metadata) %in% c("sample_id","Cluster_totalvi20240525rmigtsample_Res0.5.sample_id")]
if (any(na.omit(results<0.05))) {
    message("WARNING: IMBALANCED GROUPS FOR:")
    results[which(results<0.05)]
}
group1 = group1 %>% pull(Cluster_totalvi20240525rmigtsample_Res0.5.sample_id)
group2 = group2 %>% pull(Cluster_totalvi20240525rmigtsample_Res0.5.sample_id)
...
namescontrasts[i] = "cl1InHealthy_vs_cl2InHealthy"
names(contrasts) = namescontrasts
gene_symbol = rownames(so[["RNA"]]$counts)
