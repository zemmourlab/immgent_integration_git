...
```{r}
so = readRDS("igt1_96_miniverse_20241017.Rds")
library(reshape2)
library(pheatmap)
so_orig = so
...
so = so[,so$IGT %in% names(which(table(so$IGT) >30))]
table(so$cite_seq, so$IGT)
...
saveRDS(so, sprintf("%s/igt1_96_miniverse_filteredforCometsc.Rds", prefix))

```

Heatmap Clusters ADT
```{r}
so = readRDS(sprintf("%s/igt1_96_miniverse_filteredforCometsc.Rds", prefix))

so = NormalizeData(so, assay = "ADT", normalization.method = "CLR")
adt = so[["ADT"]]$data
tmp = as.data.frame(t(as.matrix(adt)))
tmp$miniverse_clusters = so$miniverse_clusters
tmp2 = melt(tmp)
tmp2 = tmp2 %>% group_by(miniverse_clusters, variable) %>% summarise(mean(value))
tmp2 = dcast(tmp2, variable ~ miniverse_clusters)
rownames(tmp2) = tmp2[,1]
tmp2 = tmp2[,-1]
#tmp2 = apply(tmp2,1, as.numeric)
pdf(sprintf("%s/heatmap_ADT_clusters.pdf", prefix), 10,15)
pheatmap(log10(tmp2+1), cluster_rows = F, cluster_cols = F, main = "Mean CLR ADT per miniverse_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = T, main = "Mean CLR ADT per miniverse_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = F, main = "Mean CLR ADT per miniverse_clusters")
dev.off()
...
```{bash}
cd /project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/miniverse/cometsc_20241023
...
prefix3 = "markers_miniverseclusters"
```
...
so = NormalizeData(so, assay = "ADT", normalization.method = "LogNormalize")
adt = as.matrix(so[["ADT"]]$data)
markers = c("CD62L", "KLRG1", "miniverse9B", "FR4", "SCA1", "NEUROPILIN1.CD304", "CD69", "miniverse5RB")
adt = adt[markers,]
tmp = melt(adt)
...
# pdf("markers_miniverseclusters_20240612_gatingplots.pdf", 5, 5, useDingbats = F)
# pdf("markers_miniverseclusters_20240612_gatingplots_organ.pdf", 15, 15, useDingbats = F)
...
#cl3 no pregating, just on miniverse9B pos
cl = "cl3"
feature1 = "CD62L"
feature2 = "miniverse9B"
th1 = 5
th2 = 4.5
so@meta.data[,sprintf("is_%s", cl)] = so$Cluster_totalvi20240525rmigtsample_Res0.5 %in%  cl
# FeatureScatter(so,slot = "data",feature1 = feature1, feature2 = feature2, group.by = sprintf("is_%s", cl) ) + scale_color_manual(values = c("grey", "red")) + geom_vline(aes(xintercept = th1), linetype="dashed", color = "brown") + geom_vline(aes(xintercept = th1.2), linetype="dashed", color = "brown") + geom_hline(aes(yintercept = th2), linetype="dashed", color = "brown") + ggtitle(sprintf("is_%s", cl)) 
...
for (i in unique(gates$organ_simplified)){
    ...
    # write.table(stats, file = make.names(sprintf("markers_miniverseclusters_20240612_statsSeSp_%s.tsv",i)), quote = F, row.names = T, col.names = T, sep = "\t")
