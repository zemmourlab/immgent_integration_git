...
out_dir = "~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260107" 
out_dir = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260107"

out_dir = "~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260319" 
out_dir = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/TF_analysis/20260319"
...
Gene_AUC <- function(
    ...
    if (!is.null(so) && !is.null(mypal_groups)) {
        ...
        for (g in top_genes) {
            ...
            data$annotation_level1  <- pseudobulk$annotation_level1[match(data$Group, pseudobulk$annotation_level1)]
...
so_orig = readRDS("~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107_clean.Rds")
```
...
Level1
```{r}
pseudobulk_orig = readRDS(sprintf("%s/igt1_96_pseudobulk_byclusterannotationlevel1_log1pCP10K.Rds", out_dir))

pseudobulk = pseudobulk_orig
out_dir2 = sprintf("%s/%s",out_dir, "level1")
...
pseudobulk = pseudobulk_orig
out_dir2 = sprintf("%s/%s",out_dir, "level1Top2")
Gene_AUC(pseudobulk,
         genes = tf, # vector of genes
         out_dir = out_dir2,
         mypal = mypal,              # named vector of colors for TF lines
         order_groups = levels(so_orig$annotation_level1),
         mypal_groups = mypal_level1,       # named vector of colors for groups in barplots
         n_top = 2,                # number of top TF to highlight
         expr_threshold = 0.5,       # threshold for filtering genes, in CK10K
         cols_heatmap_gradient = rev(colorRampPalette(brewer.pal(n = 7,name = "RdYlBu"))(100))
)

pseudobulk = pseudobulk_orig[,pseudobulk_orig$annotation_level1 != "Treg"]
out_dir2 = sprintf("%s/%s",out_dir, "level1_CD4noTreg")
so_orig$annotation_level1b = as.character(so_orig$annotation_level1)
so_orig$annotation_level1b[so_orig$annotation_level1b == "Treg"] = "CD4"
Gene_AUC(pseudobulk,
         genes = tf, # vector of genes
         out_dir = out_dir2,
         mypal = mypal,              # named vector of colors for TF lines
...
Across Level2
```{r}
pseudobulk_orig = readRDS(sprintf("%s/igt1_96_pseudobulk_byclusterannotationlevel2_log1pCP10K.Rds", out_dir))
...
pseudobulk = pseudobulk_orig
out_dir2 = sprintf("%s/%s",out_dir, "level2_select1")
Gene_AUC(pseudobulk,
         genes = c("Sox13", "Foxp3", "Zbtb16", "Tbx21", "Gata3", "Rorc", "Tox", "Tox2", "Hes1", "Pparg", "Tcf7"), # vector of genes
         out_dir = out_dir2,
         mypal = mypal,              # named vector of colors for TF lines
         order_groups = levels(so_orig$annotation_level2),
         mypal_groups = mypal_level1b,       # named vector of colors for groups in barplots
...
for (ct in c("CD8", "CD4", "Treg", "gdT", "CD8aa", "Tz", "DN", "DP")) {
    print(ct)
    out_dir2 = sprintf("%s/%s", out_dir, ct)
