...
dir_with_file = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/nonconv/20250109_nonconv"
...
so$level2_new[so$annotation_level2_parent1 == "proliferating"] = "nonconv_prolif"
so$level2_new[so$annotation_level2_parent1 == "miniverse"] = "nonconv_miniverse"
...
so$is_ref = "other"
so$is_ref[so$annotation_level1 == "nonconv" & so$annotation_level1_20241216 == "nonconv"] = "ref"
so$is_ref[so$annotation_level1 == "nonconv" & so$annotation_level1_20241216 != "nonconv"] = "query"
...
nonconv_cl17 in 2 clusters 
nonconv_cl27 redistributed
...
pdf(sprintf("%s/MDE_annotation_20250109_TransferLabels_Reclustering_to_see.pdf", "20250109_nonconv"), 20, 20, useDingbats = F)
...
DimPlot(so, reduction = "mde_incremental", group.by = "iNKT", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = c("blue", "red"))
DimPlot(so, reduction = "mde_incremental", group.by = "MAIT", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = c("blue", "red"))
dev.off()

so$level2_new[so$level2_new %in% "nonconv_cl27"  & so[["mde_incremental"]]@cell.embeddings[,2] > -2 ] = "nonconv_cl21"
so$level2_new[so$level2_new %in% "nonconv_cl27"  & so[["mde_incremental"]]@cell.embeddings[,2] < -2 ] = "nonconv_miniverse"
so$level2_new[so$level2_new %in% "nonconv_cl17"  & so[["mde_incremental"]]@cell.embeddings[,2] < 0 ] = "nonconv_cl55"
so$level2_new[so$level2_new %in% c("nonconv_cl8", "nonconv_cl45") ] = "nonconv_cl8"
so$level2_new[so$level2_new  %in% c("not_CD4CD8_cl48")] = "nonconv_prolif"
so$level2_new[so$annotation_level2_parent1  %in% c("proliferating")] = "nonconv_prolif"
...
so$annotation_level2_parent1[so$level2_new %in% c("nonconv_miniverse")] = "mniniverse"
so$annotation_level2_parent1[!so$level2_new %in% c("nonconv_prolif", "nonconv_miniverse", "not_CD4CD8_cl48")] = "other"
table(so$annotation_level2_parent1, so$level2_new)

pdf(sprintf("%s/MDE_annotation_20250109_level2.pdf", "20250109_nonconv"), 20, 20, useDingbats = F)
DimPlot(so, reduction = "mde_incremental", group.by = "annotation_level1", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal_level1)
DimPlot(so, reduction = "mde_incremental", group.by = "annotation_level1_20241216", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal)
...
if (all(colnames(so_orig)[so_orig$annotation_level1 == "nonconv"] == colnames(so))) {
    print("True")
    so_orig$annotation_level2[so_orig$annotation_level1 == "nonconv"] = so$level2_new[colnames(so_orig)[so_orig$annotation_level1 == "nonconv"]]
    so_orig$annotation_level2_parent1[so_orig$annotation_level1 == "nonconv"] = so$annotation_level2_parent1[colnames(so_orig)[so_orig$annotation_level1 == "nonconv"]]
}

table(so_orig$annotation_level2_parent1[so_orig$annotation_level1 == "nonconv"], so_orig$annotation_level2[so_orig$annotation_level1 == "nonconv"])
table(so_orig$annotation_level2[so_orig$annotation_level1 == "nonconv"], so_orig$annotation_level2_20241216[so_orig$annotation_level1 == "nonconv"])
...
write.table(x = annot, file = "../annotation_table_20250109_withlevel2CD8AB_CD4_gdT_nonconv.csv", row.names = F, quote = F, sep = ",")
...
pdf(sprintf("%s/Prolif_scores_MDE.pdf","."), 10, 10, useDingbats = F)
...
MyDimPlotHighlight(so_orig, umap_to_plot = "mde2_totalvi_20241006", cells_to_highlight = names(which(so_orig$annotation_level1 == "nonconv")), highlight_column_name ="annotation_level2", highlight_size = 0.5, highlight_alpha = 0.5, pixels = c(512, 512), mycols = mypal_level2, print_plot1 = T, print_plot2 = T, labelclusters = T )
...
# saveRDS(so, file = "igt1_96_nonconv_20250109.Rds")
saveRDS(so, file = "igt1_96_nonconv_20250212.Rds")
