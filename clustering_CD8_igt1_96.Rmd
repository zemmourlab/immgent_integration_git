...
so$is_ref = so$annotation_level1 == "CD8AB" & so$annotation_level1_20241216 == "CD8AB"
so$is_ref[so$annotation_level2_parent1 %in% c("miniverse", "proliferating", "preT")] = NA
cell_embeddings_ref = so[["totalvi_20241008_rmIGTsample"]]@cell.embeddings[so$is_ref,]
cell_embeddings_query = so[["totalvi_20241008_rmIGTsample"]]@cell.embeddings[!so$is_ref,]
...
labels_ref = as.character(so$annotation_level2_20241216[so$is_ref])
...
so$level2_new[!so$is_ref] = transfer_labels
