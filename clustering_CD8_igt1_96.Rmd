...
DefaultAssay(so) = "RNA"
FeaturePlot(so, slot = "data", reduction = reduc, features = c("Cd4", "Cd8a", "Cd8b1","Sell", "Ccr7", "Il7r", "Cd44", "Klrg1", "Gzmb", "Prf1", "Ifng", "Tcf7", "Bcl2", "Cxcr3", "Cxcr5", "Slamf6", "Eomes", "Foxp3","Tbx21", "Nr4a1", "Mki67", "Ifng","Pdcd1", "Havcr2", "Ctla4", "Tox", "Lag3", "Cd69", "Itgae", "Cxcr6", "Zeb2", "Runx3", "Slamf6", "Nkg7", "Zbtb16"), ncol = 6, order = T, raster = T, raster.dpi = c(512,512))
dev.off()

# pdf(sprintf("%s/UMAP_proteins.pdf", prefix), 20,20, useDingbats = F)
pdf(sprintf("%s/MDE_proteins.pdf", prefix), 30,30, useDingbats = F)
DefaultAssay(so) = "ADT"
FeaturePlot(so[,so$cite_seq == T], reduction = reduc,slot = "data", features = c("CD3","CD4","CD8A","CD8B","TCRB","IL2RA.CD25","CD62L", "CD27", "IL7RA.CD127", "CD44", "CD45RB", "KLRG1", "CD137", "CD69", "CD103", "PDCD1.PD1.CD279", "HAVCR2", "TIGIT"), ncol = 5, order = T, raster = T, raster.dpi = c(512,512), pt.size = 2)
dev.off()
```
...
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 %in% c("cl3", "cl4", "cl12")] = "CD8AB_cl1" #resting
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl9"] = "CD8AB_cl2" #resting
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl6"] = "CD8AB_cl3" #MPEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl6" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.5 == "cl17"] = "CD8AB_cl4"
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl5"] = "CD8AB_cl5" #exhausted
...
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl2"] = "CD8AB_cl7" #SLEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl2" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.5 == "cl16"] = "CD8AB_cl8" #MPEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl2" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.5 == "cl13" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res1 == "cl5"] = "CD8AB_cl9" #SLEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl2" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.5 == "cl13" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res1 == "cl27"] = "CD8AB_cl10" #SLEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl8"] = "CD8AB_cl11" #MPEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 == "cl8" & so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.5 == "cl14"] = "CD8AB_cl7" #SLEC
so$CD8AB_clusters[so$Cluster_totalvi_20241014_CD8AB_rmIGTsample_Res0.25 %in% c("cl7", "cl10", "cl11")] = "CD8AB_cl12" #MPEC
...
DimPlot(so, reduction = reduc, group.by = "CD8AB_clusters", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal)
