...
**Figure 1 - alluvial**
Alluvial organ - level2 only (change df0!)
```{r}
df0 = so_orig@meta.data %>% filter(annotation_level1 %in% "CD8" & !(annotation_level2 %in% c("CD8_prolif", "CD8_miniverse")) & !(organ_simplified0 %in% "SLO")) %>% group_by(organ_simplified0) %>%
  slice_sample(n = 100) 
...
p1 = ggplot(df1,
       aes(y = count, axis1 = organ_simplified0, axis2 = annotation_level2)) +
    coord_flip() +
    ...
    scale_x_discrete(limits = c("organ_simplified0", "annotation_level2"), expand = c(.05, .05)) +
...
p2 = ggplot(df1,
       aes(y = count, axis1 = organ_simplified0, axis2 = annotation_level2)) +
    coord_flip() +
    geom_alluvium(aes(fill = organ_simplified0), width = 1/5) +
    geom_stratum(width = 1/5, fill = "white", color = "grey") +
    geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 3, angle = 90) +
    # geom_text_repel(stat = "stratum", aes(label = after_stat(stratum)),
    #                 size = 5,
...
Alluvial condition level2 only
```{r}
...
mat_clust <- df2 %>%
  select(annotation_level2, condition_detailed_simplified, count) %>%
  pivot_wider(
    names_from  = condition_detailed_simplified,
    values_from = count,
    values_fill = 0
  ) %>% as.data.frame()
...
p3 = ggplot(df2,
       aes(y = count, axis1 = condition_detailed_simplified, axis2 = annotation_level2)) +
    coord_flip() +
    geom_alluvium(aes(fill = annotation_level2), width = 1/5) +
    geom_stratum(width = 1/5, fill = "white", color = "grey") +
    geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 3, angle = 90) +
    ...
    #                 point.padding = 0.5,    # Adjusts space around the point (stratum)
    #                 max.overlaps = Inf) +   # Allows all labels to be drawn, even if they overlap initially
    scale_x_discrete(limits = c("condition_detailed_simplified", "annotation_level2"), expand = c(.05, .05)) +
    scale_fill_manual(values = mypal) + # Or scale_fill_brewer
    ggtitle("Level2 across Conditions") +
    theme_minimal() +
    theme(
        panel.grid.major = element_blank(), # Removes major grid lines
        panel.grid.minor = element_blank(), # Removes minor grid lines
...
df0 = so_orig@meta.data %>% filter(annotation_level1 %in% "CD8" & !(annotation_level2 %in% c("CD8_prolif", "CD8_miniverse")) & !(organ_simplified0 %in% "SLO")) %>% group_by(annotation_level2, organ_simplified0, condition_detailed_simplified) %>%
  slice_sample(n = 100) 
...
mat_clust <- df1 %>%
  dplyr::select(annotation_level2, organ_simplified0, count) %>%
  pivot_wider(
    names_from  = organ_simplified0,
    values_from = count,
    values_fill = 0
...
mat_org <- df1 %>%
  dplyr::select(organ_simplified0, annotation_level2, count) %>%
  pivot_wider(
    names_from  = annotation_level2,
    values_from = count,
    values_fill = 0
...
df2 = df0%>% 
  ...
  as.data.frame()
mat_cond <- df2 %>%
  dplyr::select(condition_detailed_simplified, annotation_level2, count) %>%
  pivot_wider(
    names_from  = annotation_level2,
    values_from = count,
    values_fill = 0
...
p3 <- ggplot(
  ...
  aes(
    y     = count,
    axis1 = organ_simplified0,
    axis2 = annotation_level2,
  ...
  scale_x_discrete(
    limits = c("condition_detailed_simplified", "organ_simplified0", "annotation_level2"),
    expand = c(.05, .05)
  ) +
    coord_flip() +
...
PlotAlluvialHighlight specific clusters, conditions etc.
```{r}
library(ggplot2)
library(ggalluvial)
...
#df3 from previous block (Alluvial Pliot 3 ways)
df3_plot <- df3 %>%
  mutate(
    # is this alluvium part of the highlighted condition?
    is_highlight = T,
    # fill key: either "other" (grey) or the level2 cluster name
    fill_key = ifelse(is_highlight, as.character(condition_detailed_simplified), "other"),
    # alpha: low for background, high for highlight
    alpha_val = ifelse(is_highlight, 1, 0.1)
  )
# pal_alluv <- c(other = "grey80", mypal)  
pal_alluv <- setNames(mypal, unique(df3_plot$fill_key))
df3_plot %>% count(is_highlight)
df3_plot %>% count(fill_key)
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv )
...
df3_plot <- df3 %>%
  mutate(
    # is this alluvium part of the highlighted condition?
    is_highlight = condition_detailed_simplified %in% highlight,
    # fill key: either "other" (grey) or the level2 cluster name
...
df3_plot %>% count(is_highlight)
pal_alluv <- c(other = "grey80", mypal_level2)  # mypal_level2 is named by annotation_level2
df3_plot %>% count(fill_key)
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv )
...
pal_alluv <- c(other = "grey80", CD8_cl4 = "red")  
# pal_alluv <- setNames(mypal, unique(df3_plot$fill_key))
df3_plot %>% count(is_highlight)
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv)
...
pal_alluv <- c(other = "grey80", CD8_cl15 = "red")  
# pal_alluv <- setNames(mypal, unique(df3_plot$fill_key))
df3_plot %>% count(is_highlight)
pdf(sprintf("%s/Alluvial_CD8cl15.pdf", out_dir), 15,5, useDingbats = F)
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv)
dev.off()
...
highlight <- levels(df3$annotation_level2)
df3_plot <- df3 %>%
  mutate(
    # is this alluvium part of the highlighted condition?
    is_highlight = annotation_level2 %in% highlight,
...
df3_plot <- df3 %>%
  mutate(
    # is this alluvium part of the highlighted condition?
    is_highlight = condition_detailed_simplified %in% c("LCMVarm"),
    # fill key: either "other" (grey) or the level2 cluster name
    fill_key = ifelse(is_highlight, as.character(condition_detailed_simplified), "other"),
    # alpha: low for background, high for highlight
    alpha_val = ifelse(is_highlight, 1, 0.1)
...
df3_plot %>% count(fill_key)
pal_alluv <- c(other = "grey80", LCMVarm = "red")  
df3_plot %>% count(is_highlight)
pdf(sprintf("%s/Alluvial_CD8cl4.pdf", out_dir), 15,5, useDingbats = F)
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv )
dev.off()
...
df3_plot <- df3 %>%
  mutate(
    ...
    is_highlight = condition_detailed_simplified %in% c("LCMVarm"),
    # fill key: either "other" (grey) or the level2 cluster name
    fill_key = ifelse(is_highlight, as.character(organ_simplified0), "other"),
    # alpha: low for background, high for highlight
    alpha_val = ifelse(is_highlight, 1, 0.1)
  )
df3_plot %>% count(fill_key)
pal_alluv <- c(other = "grey80", mypal_organ)  
df3_plot %>% count(is_highlight)
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv )
...
df3_plot <- df3 %>%
  mutate(
    # is this alluvium part of the highlighted condition?
    is_highlight = condition_detailed_simplified %in% c("baseline"),
    # fill key: either "other" (grey) or the level2 cluster name
    fill_key = ifelse(is_highlight, as.character(annotation_level2), "other"),
    # alpha: low for background, high for highlight
    alpha_val = ifelse(is_highlight, 1, 0.1)
...
PlotAlluvialHighlight(df3_plot, mypal_alluv = pal_alluv )
    ...
    # build a palette: one grey for "other" + your level2 palette
...
p3 <- ggplot(
  ...
  coord_flip() +
  scale_fill_manual(values = pal_alluv) +
  scale_alpha(range = c(0.2, 1), guide = "none") +
  ggtitle("CD8: organ → cluster → condition (LCMVarm highlighted)") +
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
# df = so_orig@meta.data %>% filter(annotation_level1 %in% "CD8" & !(annotation_level2 %in% c("CD8_prolif", "CD8_miniverse"))) %>% group_by(organ_simplified) %>%
...
df = so_orig@meta.data %>% filter(annotation_level1 %in% "CD8" & !(annotation_level2 %in% c("CD8_prolif", "CD8_miniverse")) & !(organ_simplified0 %in% "SLO")) %>% group_by(organ_simplified0) %>%
  slice_sample(n = 100) %>%  # Keep no more than 5000 cells per organ_simplified0
  ungroup() %>%
  group_by(annotation_level2, organ_simplified0) %>%
  summarize(count = n(), .groups = "drop") %>%  # Count the number of cells in each group
  as.data.frame()
...
## 3. Hierarchical clustering for cluster (annotation_level2) order
mat_clust <- df %>%
  select(annotation_level2, organ_simplified0, count) %>%
  pivot_wider(
    names_from  = organ_simplified0,
...
## 4. Apply orders as factor levels
df <- df %>%
  mutate(
    organ_simplified0  = factor(organ_simplified0,  levels = organ_order),
    annotation_level2 = factor(annotation_level2, levels = cluster_order)
