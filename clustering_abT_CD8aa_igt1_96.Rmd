...
abT_CD8aa_cl56 from DN

```{r}
# FeaturePlot(so, reduction = "mde_incremental",slot = "data", features = "Cd8a", order = T, raster = T, raster.dpi = c(1024,1024))
...
pdf(sprintf("%s/MDE_annotation_20250109_TransferLabels_Reclustering_to_see.pdf", "20250109_abT_CD8aa"), 20, 20, useDingbats = F)
...
so$level2_new[so$Cluster_totalvi_20241008_rmIGTsample_Res0.5 %in% "2"] = "abT_CD8aa_cl56"
so$level2_new[so$level2_new  %in% c("not_CD4CD8_cl48")] = "abT_CD8aa_prolif"
so$level2_new[so$annotation_level2_parent1  %in% c("proliferating")] = "abT_CD8aa_prolif"
...
so$annotation_level2_parent1[!so$level2_new %in% c("abT_CD8aa_prolif", "abT_CD8aa_miniverse", "not_CD4CD8_cl48", "abT_CD8aa_prolif")] = "other"
table(so$annotation_level2_parent1, so$level2_new)

pdf(sprintf("%s/MDE_annotation_20250109_level2.pdf", "20250109_abT_CD8aa"), 20, 20, useDingbats = F)
DimPlot(so, reduction = "mde_incremental", group.by = "annotation_level1", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal_level1)
DimPlot(so, reduction = "mde_incremental", group.by = "annotation_level1_20241216", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal)
...
if (all(colnames(so_orig)[so_orig$annotation_level1 == "abT_CD8aa"] == colnames(so))) {
    ...
    so_orig$annotation_level2_parent1[so_orig$annotation_level1 == "abT_CD8aa"] = so$annotation_level2_parent1[colnames(so_orig)[so_orig$annotation_level1 == "abT_CD8aa"]]
}

table(so_orig$annotation_level2_parent1[so_orig$annotation_level1 == "abT_CD8aa"], so_orig$annotation_level2[so_orig$annotation_level1 == "abT_CD8aa"])
table(so_orig$annotation_level2[so_orig$annotation_level1 == "abT_CD8aa"], so_orig$annotation_level2_20241216[so_orig$annotation_level1 == "abT_CD8aa"])
...
write.table(x = annot, file = "../annotation_table_20250109_withlevel2CD8AB_CD4_gdT_nonconv_CD8aa.csv", row.names = F, quote = F, sep = ",")
...
so_orig$annotation_level2_parent1[so_orig$annotation_level2 %in% ""] = "proliferating"
so_orig$annotation_level2[so_orig$annotation_level2 %in% ""] = "abT_CD8aa_prolif"
...
setwd("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD8aa")
so = so_orig[,so_orig$annotation_level1 == "CD8aa"]
...
MyDimPlotHighlight(so_orig, umap_to_plot = "mde2_totalvi_20241006", cells_to_highlight = names(which(so_orig$annotation_level1 == "CD8aa")), highlight_column_name ="annotation_level2", highlight_size = 0.5, highlight_alpha = 0.5, pixels = c(512, 512), mycols = mypal_level2, print_plot1 = T, print_plot2 = T, labelclusters = T )
...
# saveRDS(so, file = "igt1_96_CD8aa_20250109.Rds")
saveRDS(so, file = "igt1_96_CD8aa_20250212.Rds")
