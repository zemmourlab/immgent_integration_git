...
Continuation from clustering_CD8aa_igt_1_96.Rmd
...
cd /project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD8aa
R
```
...
--> cometsc/20250109/igt1_96_CD8aa_filteredforCometsc.Rds

```{r}
so = readRDS("igt1_96_CD8aa_20250109.Rds")
so_orig = so
...
saveRDS(so, sprintf("%s/igt1_96_CD8aa_filteredforCometsc.Rds", prefix))

```

Heatmap Clusters ADT
```{r}
so = readRDS(sprintf("%s/igt1_96_CD8aa_filteredforCometsc.Rds", prefix))
...
```{r}
setwd("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD8aa/cometsc")
so_orig = readRDS("../../igt1_96_withtotalvi20250513_clean_ADTonly.Rds")
so = so_orig[,so_orig$cite_seq == T & so_orig$annotation_level1 == "CD8aa" &  !(so_orig$annotation_level2 %in% c("CD8aa_miniverse", "CD8aa_prolif"))]
...
features_list = list(c("CD11A", "CD73.5NTD"),
                     c("CD103", "ITB7"),
                     c("B220", "CD39"),     
                     c("B220", "CD49B"),  
                     c("CD39", "CD49B"),
...
toplot = read.table("gating/gating_toplot_20250611.csv", header = T, sep = ",", row.names = 1)
pops = read.table("gating/gating_populations_20250611.csv", header = T, sep = ",",  row.names = 1)
thresholds = read.table("gating/gating_thresholds_20250611.csv", header = T, sep = ",",  row.names = 1)
plots = read.table("gating/gating_plots_20250611.csv", header = T, sep = ",",  row.names = 1)
prefix3 = "gating/20250611"
