...
pdf(sprintf("%s/MDE_genes.pdf", prefix), 30,30, useDingbats = F)
DefaultAssay(so) = "RNA"
FeaturePlot(so, slot = "data", reduction = reduc, features = c("Cd4", "Cd8a", "Cd8b1","Foxp3","Sell", "Ccr7","Rorc","Bcl6", "Cxcr5", "Cd40lg", "Tbx21", "Il1rl1", "Nr4a1", "Mki67", "Ifng","Il4", "Il17a", "Il9", "Il22", "Il10", "Pdcd1", "Havcr2", "Ctla4", "Tox", "Ly6c1", "Lef1", "Tcf7"), ncol = 5, order = T, raster = T, raster.dpi = c(512,512))
dev.off()

# pdf(sprintf("%s/UMAP_proteins.pdf", prefix), 20,20, useDingbats = F)
pdf(sprintf("%s/MDE_proteins.pdf", prefix), 30,30, useDingbats = F)
DefaultAssay(so) = "ADT"
FeaturePlot(so[,so$cite_seq == T], reduction = reduc,slot = "data", features = c("CD3","CD4","CD8A","CD8B","TCRB","IL2RA.CD25","CD62L","CD44", "CD45RB","KLRG1", "CD69", "CD103", "CD27", "PDCD1.PD1.CD279", "HAVCR2", "TIGIT", "CXCR5", "IL7RA.CD127","IL4RA.CD124" ), ncol = 5, order = T, raster = T, raster.dpi = c(512,512), pt.size = 2)
dev.off()
```
