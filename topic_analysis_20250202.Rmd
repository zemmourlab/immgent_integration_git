...
so1 = readRDS(file = "igt1_96_withtotalvi20250109_clean.Rds")
...
spl_cells = colnames(so)[so$spleen_standard]

so@meta.data %>% filter(spleen_standard == T) %>% select(IGTHT, sample_code) %>% unique()
...
# so@meta.data %>% filter(spleen_standard == T) %>% select(IGT) %>% unique() %>% dim()
```


topic_wrapper_20250203_sampleddata_nobackfit.sh -- sampled data no backfit + K=50
topic_wrapper_20250203_sampleddata_backfit.sh -- sampled cells + backfit + K=50
topic_wrapper_20250203_alldata_nobackfit.sh -- all cells no backfit + K=50
topic_wrapper_20250203_alldata_backfit.sh -- all cells data + backfit + K=50
flashier20250215_alldata_nobackfit -- all cells  + no backfit + K=200
flashier20250215_sampleddata_nobackfit -- sampled cells + No backfit + K=200
...
# prefix2 = "flashier20250203_alldata_backfit"
# prefix2 = "flashier20250215_sampleddata_nobackfit"
...
######### former code without limma - useless
factor_means = mdata %>% filter(mdata$spleen_standard == T) %>% group_by(IGT) %>% summarize_at(vars(matches("^F")), mean) %>% as.data.frame()
factor_means = factor_means[order(extract_numeric(factor_means$IGT)),]
rownames(factor_means) = factor_means$IGT
factor_means = factor_means[,-1]
...
forms = sprintf("%s ~ 0+IGT", colnames(mdata)[grepl("^F",colnames(mdata))])
lm_list = sapply(forms, function(x) {lm(formula = as.formula(x), data = mdata[mdata$spleen_standard == T,])}, simplify = FALSE)
pv = lapply(lm_list, function(x) {
    res = summary(x)
    res$coefficients[,'Pr(>|t|)']
})
pvmat = do.call(cbind,pv)
colnames(pvmat) = colnames(mdata)[grepl("^F",colnames(mdata))]
pvmat = pvmat[order(extract_numeric(rownames(pvmat))),]
extract_numeric(rownames(pvmat)) == extract_numeric(rownames(factor_means))
labels_matrix = pvmat
labels_matrix[pvmat<0.05] = "*"
labels_matrix[pvmat>0.05] = ""
# labels_matrix = ifelse(is.na(pvmat), yes = 1, no = as.character(signif(tmp3,2)*100))
pdf(sprintf("%s/%s/spleenctrl_factors_heatmap.pdf", prefix, prefix2), 10,10, useDingbats = F)
pheatmap(factor_means, cluster_rows = F, cluster_cols = F, display_numbers = labels_matrix)
dev.off()
...
Limma with annotation level1

```{r}
prefix3 = "limma/20250218_level1_IGTHT"
...
pdf(sprintf("%s/%s/%s/heatmap.pdf", prefix, prefix2, prefix3), 10,10, useDingbats = F)
...
lm with annotation level2: 0 + annotation_level2 + IGTHT

```{r}
prefix3 = "limma/20250218_level2_IGTHT"
ensure_directory(sprintf("%s/%s/%s", prefix, prefix2, prefix3))
...
# Create the design matrix for the linear model
design = model.matrix(~ 0 + annotation_level2 + IGTHT, data = mdata) #make sure no empty category! otherwise creates an error during fit
...
```{r}
prefix3 = "limma/20250218_organsimplified_IGT"
ensure_directory(sprintf("%s/%s/%s", prefix, prefix2, prefix3))
