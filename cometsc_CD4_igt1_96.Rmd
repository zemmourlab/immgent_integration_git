...
Remove IGT90: CD45.1/2 not the panel
remove protein with less than 1000 rowSums, except TCRVA11.1-TCRVA11.2
--> cometsc_20240607/totalvi_igt1_56_allgenes_CD4_20240529_so_withclusters_filteredforCometsc.Rds
```{r}
so = readRDS("igt1_96_CD4_20241017.Rds")
library(reshape2)
library(pheatmap)
so_orig = so
...
adt = so[["ADT"]]$data
tmp = as.data.frame(t(as.matrix(adt)))
tmp$CD4_clusters = so$CD4_clusters
tmp2 = melt(tmp)
tmp2 = tmp2 %>% group_by(CD4_clusters, variable) %>% summarise(mean(value))
tmp2 = dcast(tmp2, variable ~ CD4_clusters)
rownames(tmp2) = tmp2[,1]
tmp2 = tmp2[,-1]
#tmp2 = apply(tmp2,1, as.numeric)
pdf(sprintf("%s/heatmap_ADT_clusters.pdf", prefix), 10,15)
pheatmap(log10(tmp2+1), cluster_rows = F, cluster_cols = F, main = "Mean CLR ADT per CD4_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = T, main = "Mean CLR ADT per CD4_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = F, main = "Mean CLR ADT per CD4_clusters")
dev.off()
...
sample_cells = so@meta.data %>%
  group_by(CD4_clusters) %>%
  sample_n(size = min(2000, n()), replace = FALSE) %>%
    pull(colnames)

so = so[,sample_cells]
table(so$CD4_clusters)
...
```{bash}
cd /project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD4/cometsc_20241023
