...
sinteractive --time=3:00:00 --account=pi-zemmour --partition=beagle3 --gres=gpu:1 --mem=152GB
...
**TOTAL-VI INTEGRATION for Rocky IGT 47 and 66, all genes**
```{bash}
mkdir IGT47_66_Rocky
```
Merge IGT47 and 66
```{r}
ImportIGTData = function(dir, IGT) {
    ...
    # meta = meta[,!grepl("HTO|hash.ID|outliers|singler|lowCountADT|orig.ident", colnames(meta))]
    # 
    # metadata_sample = read.table(file = sprintf("%s/%s_specific_Sample_metadata.tsv", dir, IGT), sep = "\t", header = T, as.is = T) #metadata_sample = read.table(file = sprintf("%s/%s_sample_specific_metadata.csv", dir, IGT), sep = ",", header = T, as.is = T)
    # metadata_sample = metadata_sample[,!colnames(metadata_sample) %in% "X"]
    # metadata_sample$hashtag_number = paste("HT", metadata_sample$hashtag_number, sep = "")
    # 
    # message("IGT in seurat object match IGT in specific_Sample_metadata.csv")
...
paths = list.files(path = "/project/jfkfloor2/zemmourlab/david/immgent/data/exp_id/", pattern = "dataset_clean.Rds", recursive = T, include.dirs=T)
paths = do.call(rbind, strsplit(paths, split = "\\/"))[,c(1,2)] %>% as.data.frame() %>% distinct()
colnames(paths) = c("exp_id", "IGT")
paths = paths[!paths$IGT %in% "Old",] %>% as.data.frame()
rownames(paths) = NULL
rownames(paths) = paths$IGT
#paths %>% write.table(file = "igt_exp.txt", row.names = F, quote = F, sep = ",")

so_list = list()
for (s in c("IGT47", "IGT66")) {
    #IGT = gsub(dir, pattern = "/scratch/midway3/zemmour/exp_id/\\*/", replacement = "")
    #IGT = gsub(dir, pattern = "/project/jfkfloor2/zemmourlab/david/immgent/data/", replacement = "")
    #print(IGT)
    IGT = paths[s,"IGT"]
    print(IGT)
    dir = sprintf("/project/jfkfloor2/zemmourlab/david/immgent/data/exp_id/%s/%s", paths[s,"exp_id"], paths[s,"IGT"])
    so_list[[IGT]]  = tryCatch(ImportIGTData(dir, IGT), error = function(e) {message("no data")})
}
...
so = merge(so_list[[1]], c(so_list[2:length(so_list)]), project = "immgent_IGT47_66_rocky")
so = JoinLayers(so)
saveRDS(so, file = "/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT47_66_Rocky/immgent_IGT47_66_rocky.Rds")

```


**TOTAL-VI INTEGRATION for Kaplan and Mackay all genes**
...
m = merge(dataset1.1, dataset2.1, merge.dr = F)
m = JoinLayers(m)
...
prefix = "totalvi_igt47_66_rocky_20240412"
SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sbatch $SCRIPT_DIR/run_totalvi_wrapper_20240412.sh #run the 2 following scripts: 
...
#Rscript $SCRIPT_DIR/run_totalvi_plot.R /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT47_66_Rocky totalvi_igt47_66_rocky_20240412
...
Create new seurat object with each subset and run totalvi
see github immgent_integration_git for latest code
```{r}
setwd("/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56")
SubsetData = function(so = so, to_subset = "resting_CD4", column = "for_subset") {
    tmp = so[,so$ClusterTOTALVI_Res1 %in% as.character(annot$ClusterTOTALVI_Res1[annot[,column] == to_subset])]
...
#Resting CD4
to_subset = "resting_CD4"
SubsetData(so, to_subset, column = "for_subset")
...
sprintf("Rscript run_totalvi_plots.R /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/%s/ totalvi_igt1_56_allgenes_%s_20240306", to_subset, to_subset)

#Resting CD8
to_subset = "resting_CD8"
SubsetData(so, to_subset, column = "for_subset")
#Run scripts: create the wrapper and modify the lines as follows:
prefix = sprintf("totalvi_igt1_56_allgenes_%s_20240306", to_subset)
#SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
...
#}
to_subset = "Treg"
SubsetData(so, to_subset, column = "annotation_broad")
#Run scripts: create the wrapper and modify the lines as follows:
prefix = sprintf("totalvi_igt1_56_allgenes_%s_20240306", to_subset)
#SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sprintf("sbatch $SCRIPT_DIR/run_totalvi_wrapper_20240306_%s.sh", to_subset) #run the 2 following scripts: 
...
#activated_CD4_noTreg
to_subset = "activated_CD4_noTreg"
SubsetData(so, to_subset, column = "annotation_broad")
#Run scripts: create the wrapper and modify the lines as follows:
prefix = sprintf("totalvi_igt1_56_allgenes_%s_20240306", to_subset)
#SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sprintf("sbatch $SCRIPT_DIR/run_totalvi_wrapper_20240306_%s.sh", to_subset) #runs the 2 following scripts, modifies those lines : 
sprintf("Rscript run_totalvi.R /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/%s/ immgent_IGT1_56_%s.Rds totalvi_igt1_56_allgenes_%s_20240306", to_subset, to_subset, to_subset)
sprintf("Rscript run_totalvi_plot.R /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/%s/ totalvi_igt1_56_allgenes_%s_20240306", to_subset, to_subset)
```

Plot the ClusterTOTALVI_Res1 on the different subset UMAPs
...
MyPlots = function (seurat_object = so, dim1 = so[["umap_unintegrated"]]@cell.embeddings[,1], dim2 = so[["umap_unintegrated"]]@cell.embeddings[,2], color_by = "spleen_standard", split_by1 = "IGT", split_by2 =  NULL, genes = c("Foxp3", "Il2ra"), cluster_key = "ClusterSCVI_Res", mypal = glasbey()) {
    ...
    plot1 = ggplot(data.frame(so@meta.data, dim1 = dim1, dim2 = dim2)) + 
        geom_point_rast(aes(dim1, dim2, color = color_by), alpha = I(alpha), raster.dpi = 100) +
        theme_bw() + scale_color_manual(values = mypal) + ggtitle(label = sprintf("color: %s", color_by))
    print(plot1)
...
prefix = "totalvi_igt1_56_20231030_allgenes"
alldata = readRDS(file = sprintf("bpcells/%s_counts/%s_BPCells_so.Rds", prefix, prefix))

to_subset_list = c("activated_CD4", "activated_CD4_noTreg", "Treg", "activated_CD8", "resting_CD4", "resting_CD8", "bottom_right", "unconventional")

for (i in 1:length(to_subset_list)) {
    
    print(to_subset_list[i])
    
    so = readRDS(sprintf("%s/totalvi_igt1_56_allgenes_%s_20240306_so.Rds",to_subset_list[i], to_subset_list[i]))
    any(is.na(alldata@meta.data[colnames(so),"ClusterTOTALVI_Res1"]))
    so$ClusterTOTALVIRes1_all = alldata@meta.data[colnames(so),"ClusterTOTALVI_Res1"]
    so$ClusterTOTALVIRes2_all = alldata@meta.data[colnames(so),"ClusterTOTALVI_Res2"]
    so$ClusterTOTALVIRes3_all = alldata@meta.data[colnames(so),"ClusterTOTALVI_Res3"]
    so$ClusterTOTALVIRes4_all = alldata@meta.data[colnames(so),"ClusterTOTALVI_Res4"]
    
    pdf(sprintf("%s/totalvi_igt1_56_allgenes_%s_20240306_UMAP_previousClusterTOTALVIRes.pdf", to_subset_list[i], to_subset_list[i]), 20, 20, useDingbats = F)
    MyPlots(seurat_object = so, 
            dim1 = so[["umap_totalvi"]]@cell.embeddings[,1], 
            dim2 = so[["umap_totalvi"]]@cell.embeddings[,2], 
            color_by = "ClusterTOTALVIRes1_all", 
            split_by1 = NULL,
            split_by2 =  NULL, 
            genes = NULL, 
    ...
    MyPlots(seurat_object = so, 
            dim1 = so[["umap_totalvi"]]@cell.embeddings[,1], 
            dim2 = so[["umap_totalvi"]]@cell.embeddings[,2], 
            color_by = "ClusterTOTALVIRes2_all", 
            split_by1 = NULL,
            split_by2 =  NULL, 
            genes = NULL, 
            cluster_key = NULL, mypal = mypal) #glasbey()
    ...
    MyPlots(seurat_object = so, 
            dim1 = so[["umap_totalvi"]]@cell.embeddings[,1], 
            dim2 = so[["umap_totalvi"]]@cell.embeddings[,2], 
            color_by = "ClusterTOTALVIRes3_all", 
            split_by1 = NULL,
            split_by2 =  NULL, 
            genes = NULL, 
            cluster_key = NULL, mypal = mypal) #glasbey()
    ...
    MyPlots(seurat_object = so, 
            dim1 = so[["umap_totalvi"]]@cell.embeddings[,1], 
            dim2 = so[["umap_totalvi"]]@cell.embeddings[,2], 
            color_by = "ClusterTOTALVIRes4_all", 
            split_by1 = NULL,
            split_by2 =  NULL, 
            genes = NULL, 
            cluster_key = NULL, mypal = mypal) #glasbey()
...
**TOTAL-VI INTEGRATION IGT 1-56, all genes, removing organ effect using PYTHON SCRIPT**
-->totalvi_igt1_56_20231030_allgenes_organregressed

```{bash}
SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sbatch $SCRIPT_DIR/run_totalvi_wrapper_20240410_rmorgan.sh
...
**Run totalVI with ALL CELLS batch_key=IGT and categorical_covariate_keys=sample_id_david: totalvi_igt1_56_allgenes_sampleregressedout using PYTHON SCRIPT**
--> totalvi_igt1_56_allgenes_20240526_igtsampleregressedout
all cells
```{bash}
SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sbatch $SCRIPT_DIR/run_totalvi_wrapper_20240527.sh
#SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
#working_dir=/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/
#path_to_mudata=/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/export_data/totalvi_igt1_56_20231030_allgenes_mdata.h5mu
#prefix=totalvi_igt1_56_allgenes_20240526_igtsampleregressedout
...
**Run totalVI with ALL CELLS batch_key=IGT and categorical_covariate_keys=sample_id_david on T cells only and removing thymus**
Run removing non T cells and thymus
-->totalvi_igt1_56_tcellonly_nothymus_igtsampleregressedout
```{bash}
cd /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/
```
...
mdata2 = mdata.copy()
rm_clusters = ["32","34","41","49","37","13","28","38","48","50","54"]
#mask = ~mdata.mod['RNA'].obs['ClusterTOTALVI_Res1'].isin(rm_clusters)
mdata2.mod['RNA'] = mdata2.mod['RNA'][ ~mdata2.mod['RNA'].obs['ClusterTOTALVI_Res1'].isin(rm_clusters)].copy()
print(mdata2.mod['RNA'].obs['ClusterTOTALVI_Res1'].unique())
mdata2.update()
mu.pp.intersect_obs(mdata2)

mdata2.write("/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/adata_tcellonly_nothymus.h5mu")
```

```{bash}
SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sbatch $SCRIPT_DIR/run_totalvi_wrapper_20240527_2.sh
##run the following scripts: 
#SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
#working_dir=/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/
...
**Add new dims in seurat object Tregs**
--> totalvi_igt1_56_allgenes_Treg_20240528_so.Rds
...
update the metadata in seurat object : see metadata_update.Rmd

Add new scvi dims in R Treg object
```{r}
setwd("/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/Treg")

# sprintf("%s_X_SCVI_%s.csv", prefix, prefix2)

AddSCVILatent = function(so, latent_file, prefix) {
...
so[["umap_totalvi_20230306_rmIGT"]] = so[["umap_totalvi"]] 
so[["totalvi_20230306_rmIGT"]] = so[["totalvi"]]

so = AddSCVILatent(so, latent_file = "totalvi_igt1_56_allgenes_Treg_20240525_igtsampleregressedout/latent.csv", prefix = "totalvi_20240525_rmigtsample")
write.table(so[['umap_totalvi_20240525_rmigtsample']]@cell.embeddings, file = "totalvi_igt1_56_allgenes_Treg_20240525_igtsampleregressedout/umap.csv", quote = F, sep = ",", row.names = T, col.names = T)
...
prefix = "totalvi_igt1_56_allgenes_Treg_20240525_igtsampleregressedout"
umap_to_plot = "umap_totalvi_20240525_rmigtsample"

pdf(sprintf("%s_UMAP_test.pdf", prefix), 20, 20, useDingbats = F)
MyPlots(seurat_object = so, 
...
MyPlots(seurat_object = so, 
        ...
        split_by2 =  NULL, 
        genes = c("Foxp3", "Ccr7","Rorc", "Tbx21", "Pparg","Il1rl1", "Nr4a1", "Mki67", "Il10", "Il2ra", "Ctla4", "Ly6c1"), 
        cluster_key = NULL, mypal = mypal) #glasbey()

dev.off()

pdf(sprintf("%s_UMAP_conditionXorgan_test.pdf", prefix), 30, 90, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
        color_by = "hashtag_number", 
...
pdf(sprintf("%s_UMAP_genes.pdf", prefix), 10, 10, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
        color_by = "organ_simplified", 
...
umap_to_plot = "mde_totalvi_20241006"
...
pdf("mde2_totalvi_20241006_topdistortedcells.pdf", 20, 20, useDingbats = F)
DimPlot(so, reduction = red, group.by = "highdistortion", order = T, raster = T, raster.dpi = c(1024,1024), label = F) + scale_color_manual(values = c("grey", "red"))
dev.off()
...
pdf(sprintf("%s_MDE_organs.pdf", prefix), 20, 20, useDingbats = F)
...
pdf(sprintf("%s_UMAP_organs_detailed_gut.pdf", prefix), 20, 20, useDingbats = F)
...
pdf(sprintf("%s_MDE_clusters.pdf", prefix), 20, 20, useDingbats = F)
p1 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 0.25), raster = T, raster.dpi = c(1024,1024), label = T) + scale_color_manual(values = mypal)
p2 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 0.5), raster = T, raster.dpi = c(1024,1024), label = T) + scale_color_manual(values = mypal)
p2.2 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 0.75), raster = T, raster.dpi = c(1024,1024), label = T) + scale_color_manual(values = mypal)
p3 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 1), raster = T, raster.dpi = c(1024,1024), label = T) + scale_color_manual(values = mypal)
p4 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 2), raster = T, raster.dpi = c(1024,1024), label = T) + scale_color_manual(values = mypal)
p5 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 3), raster = T, raster.dpi = c(1024,1024), label = T ) + scale_color_manual(values = mypal)
p6 = DimPlot(so, reduction = umap_to_plot, group.by = sprintf("Cluster%s_Res%s", prefix, 4), raster = T, raster.dpi = c(1024,1024), label = T) + scale_color_manual(values = mypal)
print(p1) #+ NoLegend()
print(p2)
print(p2.2)
print(p3)
...
#pdf(sprintf("%s_UMAP_genes3.pdf", prefix), 20,20, useDingbats = F)
pdf(sprintf("%s_MDE_genes.pdf", prefix), 20,20, useDingbats = F)
...
DefaultAssay(so) = "ADT"
#pdf(sprintf("%s_UMAP_proteins.pdf", prefix), 20,20, useDingbats = F)
pdf(sprintf("%s_MDE_proteins.pdf", prefix), 20,20, useDingbats = F)
...
FeaturePlot(so[,so$cite_seq == T], reduction = umap_to_plot, features = "TCRVG1.1", raster = , order = F, raster.dpi = c(1024,1024))
...
pdf(sprintf("%s_UMAP_annotationlevel1_genes.pdf", prefix), 40,40, useDingbats = F)
DefaultAssay(so) = "RNA"
...
pdf(sprintf("%s_UMAP_annotationlevel1_proteins.pdf", prefix), 40,40, useDingbats = F)
DefaultAssay(so) = "ADT"
