...
**Initialize: load libraries and custom functions (ok)**
...
out_dir = "~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/misc" 
out_dir = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/misc"
...
*FeatureScatter ADT (Fig 2 / S3) (ok)*
```{r}
...
**Colon in IGT20, IGT24 and IGT27  (Fig 2) (ok)**
```{r}
...
**Level2 in lineage-specific MDE - Figure 3a**
```{r}
so = so_orig[,so_orig@meta.data %>% filter(organ_simplified != "thymus") %>% row.names()]
...
genes <- c("Cd3e", "Cd8a", "Cd8b1", "Cd4", "Trdc", "Zbtb16", "Foxp3", "Rorc", "Sell", "Cd44", "Itgae", "Ifng", "Il4", "Il17a", "Gzma", "Pdcd1", "Klrg1")
genes_rev <- rev(genes)

pdf(sprintf("%s/3c_Dotplot_genes_alldata_nothymocyte_data_viridis.pdf", out_dir), height = 8, width = 25, useDingbats = F)
Seurat::DotPlot(so, features = genes_rev, group.by = "annotation_level2", scale = F) + coord_flip() + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_color_viridis_c(option = "C")
dev.off()
...
pdf(sprintf("%s/4a_CumulativeNumberClusters_immgenT.pdf", out_dir,i), 10,5, useDingbats = F)
p = ggplot(igt_steps, aes(x = IGT, y = cum_clusters, group = 1)) +
  geom_line() +
  geom_point(size = 1.5) +
    geom_hline(yintercept = 107, colour = "brown", linetype = "dashed", size = 0.5) +
...
**Fig 4b- Biological conservation analysis: Cluster structure conservation with silhouette analysis**
```{r}
library(cluster)
...
sil_summary_list = readRDS("%s/BioConservationSilhouette_IGT.Rds")
...
**Fig 4c - Cluster Separability in RNA Space (Midway)**
```{r}
...
# saveRDS(sil_summary_list, file = sprintf("%s/ClusterSeparation_RNA_IGT.Rds", out_dir))
...
pdf(sprintf("%s/4c_Boxplot_ClusterSeparability_Level2RNASpace.pdf", out_dir), 25,6, useDingbats = F)
p
dev.off()
...
**S4: Cluster robustness**

S4a: Clusters by IGT (ok)
```{r}
library(dplyr)
library(rlang)
...
pdf(sprintf("%s/S4a_Barplot_Level2ByIGT.pdf", out_dir), 15,5, useDingbats = F)
p + facet_grid(
        . ~ annotation_level1,
        scales = "free_x",
        space  = "free_x"
...
S4d:Cluster Separability in ADT Space (ok)
```{r}
library(cluster)
so = so_orig[,so_orig@meta.data %>% filter(organ_simplified != "thymus", cite_seq == T) %>% row.names()]
so$level2_int = as.integer(so[["annotation_level2"]][,1])
...
saveRDS(sil_summary_list, file = sprintf("%s/ClusterSeparation_ADT_IGT.Rds", out_dir))
...
pdf(sprintf("%s/S4d_Boxplot_ClusterSeparability_Level2ADTSpace.pdf", out_dir), 25,6, useDingbats = F)
p
dev.off()
...
**Fig 6a - S6 - MDE plots organ (Fig S6) (ok)**
```{r}
tmp = so[,so@meta.data %>% filter(condition_broad == "healthy" & organ_simplified0 != "thymus") %>% row.names()]

pdf(sprintf("%s/6c_MDE_organ_separated_collevel.pdf", out_dir), 6,6, useDingbats = F)
...
**Figure 6 -  Proportion of level2 (denominator is level1) in each organ**
...
make_comp_dotdata_and_plot <- function(data,
    ...
    # move baseline to top if present
...
so = so_orig[,so_orig@meta.data %>% filter(organ_simplified != "thymus", cite_seq == T) %>% row.names()]
m3 = readRDS(file = sprintf("%s/annotation_level2_PropPerSamplePerLevel1_withMetadata.Rds", '/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/SampleComp/20260206'))
...
pdf(sprintf("%s/6b_DotPlot_Level2ByOrgan_healthy.pdf", out_dir), 25,5, useDingbats = F)
...
pdf(sprintf("%s/6d_DotPlot_Level2ByConditionDetailedSimplified_Lung.pdf", out_dir), 22,3.5, useDingbats = F)
...
pdf(sprintf("%s/6c_DotPlot_Level2ByConditionDetailedSimplified_Colon.pdf", out_dir), 22,3.5, useDingbats = F)
...
write.table(final_table, file = sprintf("%s/Extended_Data_Table_S1.txt", out_dir), sep = "\t", quote = FALSE, row.names = FALSE)
```


**Table S2: DGE for each cluster**
```{r}
library(dplyr)
library(readr)
library(purrr)
...
base_dir <- '/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/20260206_freeze/DGE_Limma'  # change this

folders <- list.dirs(base_dir, recursive = FALSE, full.names = TRUE) %>%
  keep(~ str_detect(basename(.x), "^OnevsAllinLineage_"))

for (folder in folders[7:8]) {
  
  lineage <- str_remove(basename(folder), "OnevsAllinLineage_")
  # e.g. "CD4", "Treg", "CD8", etc.
  print(lineage)
  ...
  combined <- map_dfr(files, function(f) {
    ...
    sig <- str_remove(basename(f), "\\.txt$")
    
    # insert lineage after "All"
    sig_new <- str_replace(sig, "_vs_All_", paste0("_vs_All", lineage, "_"))
    
    read_tsv(f, show_col_types = FALSE) %>%
      mutate(signature = sig_new)
  })
  
  out_file <- file.path(
    folder,
    paste0(basename(folder), "_combined.txt")
  )
