...
annotation_level2_CD4_activated.csv
```{r}
library(cluster)
cluster.name = sprintf("Cluster_%s_Res0.25",prefix)
...
so$CD4_activated_clusters = "unknown"
so$CD4_activated_clusters[so$Cluster_totalvi_20241014_CD4_activated_rmIGTsample_Res0.25 == "cl1"] = "CD4_activated_cl1" #resting
so$CD4_activated_clusters[so$Cluster_totalvi_20241014_CD4_activated_rmIGTsample_Res0.25 %in% c("cl3", "cl4", "cl12")] = "CD4_activated_cl1" #resting
so$CD4_activated_clusters = factor(so$CD4_activated_clusters)
table(so$CD4_activated_clusters)

# so$test = so$CD4_activated_clusters %in% "unknown"
