...
Continuation from clustering_Tregs1_96.Rmd
...
Filter the data:
all data with CITEseq
Remove IGT1: CITEseq V1
Remove IGT7,8,9,44,45,46: no ADT
remove protein with less than 1000 rowSums, except TCRVA11.1-TCRVA11.2
--> cometsc_20240607/totalvi_igt1_56_allgenes_Treg_20240529_so_withclusters_filteredforCometsc.Rds
```{r}
so = readRDS("igt1_96_treg_20241017.Rds")
library(reshape2)
library(pheatmap)
so_orig = so
...
adt = so[["ADT"]]$data
tmp = as.data.frame(t(as.matrix(adt)))
tmp$Treg_clusters = so$Treg_clusters
tmp2 = melt(tmp)
tmp2 = tmp2 %>% group_by(Treg_clusters, variable) %>% summarise(mean(value))
tmp2 = dcast(tmp2, variable ~ Treg_clusters)
rownames(tmp2) = tmp2[,1]
tmp2 = tmp2[,-1]
#tmp2 = apply(tmp2,1, as.numeric)
pdf(sprintf("%s/heatmap_ADT_clusters.pdf", prefix), 10,15)
pheatmap(log10(tmp2+1), cluster_rows = F, cluster_cols = F, main = "Mean CLR ADT per Treg_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = T, main = "Mean CLR ADT per Treg_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = F, main = "Mean CLR ADT per Treg_clusters")
dev.off()
...
```{r}
prefix = prefix
prefix2 = "20241023_k2_logdata" #"20241023_k2data" 
prefix3 = "markers_Tregclusters"
```
...
# pdf("markers_Tregclusters_20240612_gatingplots.pdf", 5, 5, useDingbats = F)
# pdf("markers_Tregclusters_20240612_gatingplots_organ.pdf", 15, 15, useDingbats = F)
...
for (i in unique(gates$organ_simplified)){
    ...
    # write.table(stats, file = make.names(sprintf("markers_Tregclusters_20240612_statsSeSp_%s.tsv",i)), quote = F, row.names = T, col.names = T, sep = "\t")
