...
# python3 $1/train_query_model_v3.py [query_data_path] [protein_path] [integrated_data_path] [subgroup] [celltype_label]
...
query_data_path = sys.argv[1]
protein_path = sys.argv[2]
integrated_data_path = sys.argv[3]
subgroup = sys.argv[4]
celltype_label = sys.argv[5]



# read in created matrix
matrix_query = mmread(query_data_path+"/matrix/matrix.mtx")
genes = pd.read_csv(query_data_path+"/matrix/genes.tsv", delimiter = "\t",header=None)
cells_query = pd.read_csv(query_data_path+"/matrix/barcodes.tsv",header=None)
proteins = pd.read_csv(protein_path+"/proteins.tsv",header=None)
...
# read in mudata
adata = mu.read(integrated_data_path+"/model/adata.h5mu") #load this one (results of this block)
print(adata)
adata.mod["RNA"].layers["counts"] = adata.mod["RNA"].X.copy()

# add annotation level 1 and 2, then subset
annotation_level1 = pd.read_csv(integrated_data_path+"/annotation_level1.csv", delimiter = ",", header = None)
annotation_level2 = pd.read_csv(integrated_data_path+"/annotation_level2.csv", delimiter = ",")
...
adata = adata[adata.obs['annotation_level1'] == subgroup]
adata.mod["RNA"].layers["counts"] = adata.mod["RNA"].X.copy()
...
# read in model
adata = adata.copy()
model = scvi.model.TOTALVI.load(integrated_data_path+"/model", adata=adata)
model = scvi.model.TOTALVI.load(integrated_data_path+"/model", adata=adata)
...
query_metadata = pd.read_csv(query_data_path+"/cell_metadata.csv", delimiter = ",")
query_metadata.index = mdata.obs_names.astype(str)
mdata.obs['annotation_level2'] = query_metadata["celltype_label"].values
...
mdata.obs['annotation_level2'] = query_metadata['celltype_label'].values
mdata.mod['RNA'].obs['annotation_level2'] = query_metadata['celltype_label'].values
...
full.obs['annotation_level2'][len(adata.obs_names):] = query_metadata['celltype_label'].values
#adata.obs['annotation_level2'].values + query_metadata['celltype_label'].values
full.obs['clusters/celltype_label'] = "NA"
clusters_celltypes = pd.concat([adata.obs['annotation_level2'],query_metadata['celltype_label']])
