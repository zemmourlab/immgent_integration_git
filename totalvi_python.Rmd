...
```
 ...
 muData object: 
--> totalvi_igt1_56_allgenes_Treg_20240306/adata.h5mu
```{python}
mdata = mu.read("totalvi_igt1_56_allgenes_Treg_20240306/adata.h5mu") #load this one (resutls of this block)
...
**Run totalVI with organ_simplified regressed out: totalvi_igt1_56_allgenes_Treg_20240327_organregressedout**
```{python}
#Setup mu_data
...
**Run totalVI with batch_key=IGT and categorical_covariate_keys=sample_id_david: totalvi_igt1_56_allgenes_Treg_20240525_sampleregressedout**
```{python}
#Setup mu_data

(mdata.mod['RNA'].obs["sample_id"].values.isna()).any()

scvi.model.TOTALVI.setup_mudata(
    mdata,
    rna_layer="counts",
    categorical_covariate_keys = ['sample_id'], #make sure this is a list!
    protein_layer=None,
    batch_key='IGT',
    modalities={
        "rna_layer": "RNA",
...
#save latent
latent_representation = model.get_latent_representation()
TOTALVI_LATENT_KEY = "X_totalVI_igtsample"
mdata.mod['RNA'].obsm[TOTALVI_LATENT_KEY] = latent_representation
latent_df = pd.DataFrame(latent_representation, index = mdata.mod['RNA'].obs.index)
...
#save corrected counts --> see below
...
**totalvi_igt1_56_allgenes_Treg_20240306**

add TOTALVI space and protein denoised to totalvi_igt1_56_allgenes_Treg_20240306
...
Get corrected count data (for RNA and protein)
...
model.adata.var_names + model.adata.obsm['protein'].columns

genes_prot = pd.DataFrame(model.adata.var_names.append(model.adata.obsm['protein'].columns))
genes_prot = pd.DataFrame(model.adata.var_names)
genes_prot.to_csv(prefix+"/genes.csv", index = False,header=False )

cells = pd.DataFrame(model.adata.obs_names)
cells.to_csv(prefix+"/cells.csv", index = False,header=False )

```

**Run totalVI with ALL CELLS batch_key=IGT and categorical_covariate_keys=sample_id_david: totalvi_igt1_56_allgenes_sampleregressedout**
...
```{python}
os.chdir("/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT1_56/")
prefix = "totalvi_igt1_56_allgenes_20240526_igtsampleregressedout"

mdata = mu.read("export_data/totalvi_igt1_56_20231030_allgenes_mdata.h5mu") 
