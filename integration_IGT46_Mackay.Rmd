---
title: "integration IGT46 Mackay"
...
cd /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/IGT46_Mackay
```
...
source activate /project/jfkfloor2/zemmourlab/david/envs/scvi_20240315
module load openblas/0.3.13 #load again or error
...
TOTALVI_LATENT_KEY = "X_SCVI"
adata.obsm[TOTALVI_LATENT_KEY] = latent_representation
latent_df = pd.DataFrame(latent_representation, index = adata.obs.index)

latent_df.to_csv(prefix+"_X_SCVI.csv", index=True)
adata.write(prefix+"_adata_SCVI.h5ad")

```
Add new totalvi dim in R
```{r}
totalvi = read.csv(file = sprintf("%s_X_SCVI.csv", prefix), header = T, sep = ",", row.names = 1)
# pattern = "(IGT\\S*)_(\\d+)"
...
so[["totalvi"]] = CreateDimReducObject(embeddings = totalvi, key = "totalvi_", assay = "RNA")

so = RunUMAP(so, dims = 1:ncol(totalvi), reduction = "totalvi", n.components = 2, reduction.name = "umap_totalvi")

so$sample_name[is.na(so$sample_name)] = so$group[is.na(so$sample_name)]
any(is.na(so$sample_name))
