...
Continuation from clustering_gdT_igt_1_96.Rmd
...
--> cometsc/20250109/igt1_96_gdT_filteredforCometsc.Rds

```{r}
so = readRDS("igt1_96_gdT_20250109.Rds")
so_orig = so
...
saveRDS(so, sprintf("%s/igt1_96_gdT_filteredforCometsc.Rds", prefix))

```

Heatmap Clusters ADT
```{r}
so = readRDS(sprintf("%s/igt1_96_gdT_filteredforCometsc.Rds", prefix))
...
```{r}
setwd("/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/analysis/data_integration/IGT1_96/gdT/cometsc/")
so_orig = readRDS("../../igt1_96_withtotalvi20250513_clean_ADTonly.Rds")
so = so_orig[,so_orig$cite_seq == T & so_orig$annotation_level1 == "gdT" &  !(so_orig$annotation_level2 %in% c("gdT_miniverse", "gdT_prolif"))]
...
```{r}
setwd("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/gdT/cometsc")
so_orig = readRDS("../../igt1_96_withtotalvi20250513_clean_ADTonly.Rds")
so = so_orig[,so_orig$cite_seq == T & so_orig$annotation_level1 == "gdT" &  !(so_orig$annotation_level2 %in% c("gdT_miniverse", "gdT_prolif"))]
...
pdf(sprintf("%s/heatmap_ADT.pdf", "cometsc"), 10,15)
...
features_list = list(c("CD8A", "CD11A"), #1
                     c("TCRVG1.1", "TCRVG3"),
                     c("TCRVG1.1", "TCRVG2"),     
                     c("B220", "ITAX.CD11C"),  
                     c("CD39", "CD200R"), #5
...
FeatureScatter(so,slot = "data",feature1 = feature1, feature2 = feature2, group.by = "annotation_level2", split.by = "annotation_level2", ncol = 4, raster = F, pt.size = 0.2) + scale_color_manual(values = mypal_level2) + geom_vline(aes(xintercept = th1), linetype="dashed", color = "brown") + geom_hline(aes(yintercept = th2), linetype="dashed", color = "brown") 
...
FeatureScatter(so,slot = "data",feature1 = feature1, feature2 = feature2, group.by = "annotation_level2", split.by = "annotation_level2", ncol = 4, raster = F, pt.size = 0.2) + scale_color_manual(values = mypal_level2) + geom_vline(aes(xintercept = th1), linetype="dashed", color = "brown") + geom_hline(aes(yintercept = th2), linetype="dashed", color = "brown")#+ ggtitle(sprintf("is_%s", cl)) 
...
toplot = read.table("gating/gating_20250616_toplot.csv", header = T, sep = ",", row.names = 1, as.is = T)
pops = read.table("gating/gating_20250616_populations.csv", header = T, sep = ",",  row.names = 1, as.is = T)
thresholds = read.table("gating/gating_20250616_thresholds.csv", header = T, sep = ",",  row.names = 1, as.is = T)
plots = read.table("gating/gating_20250616_plots.csv", header = T, sep = ",",  row.names = 1, as.is = T)
prefix3 = "gating/20250616"

toplot = read.table("gating/gating_20250624_toplot.csv", header = T, sep = ",", row.names = 1, as.is = T)
pops = read.table("gating/gating_20250624_populations.csv", header = T, sep = ",",  row.names = 1, as.is = T)
thresholds = read.table("gating/gating_20250624_thresholds.csv", header = T, sep = ",",  row.names = 1, as.is = T)
plots = read.table("gating/gating_20250624_plots.csv", header = T, sep = ",",  row.names = 1, as.is = T)
prefix3 = "gating/20250624"

toplot = read.table("gating/gating_20250626_toplot.csv", header = T, sep = ",", row.names = 1, as.is = T)
pops = read.table("gating/gating_20250626_populations.csv", header = T, sep = ",",  row.names = 1, as.is = T)
thresholds = read.table("gating/gating_20250626_thresholds.csv", header = T, sep = ",",  row.names = 1, as.is = T)
plots = read.table("gating/gating_20250626_plots.csv", header = T, sep = ",",  row.names = 1, as.is = T)
prefix3 = "gating/20250626"

toplot = read.table("gating/gating_202506262_toplot.csv", header = T, sep = ",", row.names = 1, as.is = T)
pops = read.table("gating/gating_202506262_populations.csv", header = T, sep = ",",  row.names = 1, as.is = T)
thresholds = read.table("gating/gating_202506262_thresholds.csv", header = T, sep = ",",  row.names = 1, as.is = T)
plots = read.table("gating/gating_202506262_plots.csv", header = T, sep = ",",  row.names = 1, as.is = T)
prefix3 = "gating/202506262"

#Testing Halgouret et al strategy
toplot = read.table("gating/gating_20250628_toplot.csv", header = T, sep = ",", row.names = 1, as.is = T)
pops = read.table("gating/gating_20250628_populations.csv", header = T, sep = ",",  row.names = 1, as.is = T)
thresholds = read.table("gating/gating_20250628_thresholds.csv", header = T, sep = ",",  row.names = 1, as.is = T)
plots = read.table("gating/gating_20250628_plots.csv", header = T, sep = ",",  row.names = 1, as.is = T)
prefix3 = "gating/20250628"
...
for (i in 1:nrow(pops)) { #
    pop = rownames(pops)[i]
    print(pop)
    toplots = colnames(toplot)[!is.na(toplot[pop,])] %>% gsub(pattern = "\\.", replacement = "-")
    ...
    for (j in toplots) { #j = toplots[8]
        print(j)
        features = colnames(plots)[!is.na(plots[j,])] %>% gsub(pattern = "\\.", replacement = "-")
        feature1 = features[1]
        feature2 = features[2]
...
for (i in 1:nrow(pops)) {
    ...
    names(mrks) = names(mrks) %>% gsub(pattern = "\\.", replacement = "-")
