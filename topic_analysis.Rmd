...
The ‘fastTopics’ approach aims to combine the advantages of Poisson NMF and topic modeling.
The Poisson NMF is easier to fit because it doesn’t have the additional constraints (rows of L and columns of F sum to 1, i.e. probabilities).
On ther other hand, the parameters of the topic model are typically more useful for for understanding data; for example, the 𝑙∗𝑖𝑘
’s are topic proportions, which can be used to compare samples (documents), among other things.
...
(1) initialize the Poisson NMF model fit (‘init_poisson_nmf’); 
(2) perform the main model fitting step by running 100 EM updates using ‘fit_poisson_nmf’; 
(3) refine the fit by running 100 extrapolated SCD updates, again using ‘fit_poisson_nmf’; and 
(4) recover the multinomial topic model by calling ‘poisson2multinom’.
...
‘fastTopics’ extends the standard DE analysis to allow for partial membership to multiple groups (the methods are described here). 

lfc.stat = "vsnull: compare against the probability under the null model
lfc.stat = k: compare against a chosen topic, k
lfc.stat = "le": default, computes the “least extreme” LFC, defined as LFC(j) = log2(f_j/f_k) such that k is the topic other than j that gives the ratio f_j/f_k closest to 1.
```{r}
set.seed(1)
...
We use the following rule of thumb: estimate the standard deviation of the transformed data for a Poisson random variable with rate 𝜇=1/, where 𝑛is the number of samples. This standard deviation corresponds to a gene for which we would expect to observe a single count across all 𝑛cells, so it can serve as a reasonable lower bound to prevent variance estimates from getting too small.
```{r}
...
One alternative to NMF that could potentially reveal other interesting substructures is a semi-non-negative matrix factorization (semi-NMF). This is achieved by assigning different priors to 𝐋 and 𝐅: a prior with non-negative support (such as the point-exponential prior) for 𝐋; and a prior with support for all real numbers for  (such as the point-Laplace prior). The call to the “flash” function looks the same as above except for the “ebnm_fn” argument:

Specifying the number of factors 𝐾is not needed because flashier automatically
