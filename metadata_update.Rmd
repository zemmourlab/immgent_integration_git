...
m = read.table("/project/zemmour/david/ImmgenT/exp_id/IGT1_96_sample_metadata.csv",header = T, sep = ",")
```

update the metadata in seurat object
to do: missing metadata about IGT21HT1 and IGT43HT1 and IGT57+
```{r}
...
m$cite_seq = T
m$cite_seq[m$IGT %in% sprintf("IGT%s",c(7,8,9,44,45,46,73,85,91,92))] = F
...
```{r}
so$cite_seq = T
so$cite_seq[so$IGT %in% sprintf("IGT%s",c(7,8,9,44,45,46,73,85,91,92))] = F
...
DefaultAssay(so) = "RNA"
so[['ADT']]$data = NULL
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20241016.Rds")
saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20241021.Rds")
