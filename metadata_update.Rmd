...
so@meta.data = data.frame(IGT_cellID = colnames(so))
rownames(so@meta.data) = so$IGT_cellID
```
...
Add mde_incremental in the whole object
```{r}
...
tmp = merge(so@meta.data, scqc, by = "IGT_cellID", all.x = T, all.y = F)
...
# all(colnames(so) == tg$IGT.cellID[match(colnames(so),tg$IGT.cellID)])
so$transgenic_byTCR = tg$TG[match(colnames(so),tg$IGT.cellID)]
all(so$transgenic_byTCR[tg$IGT.cellID] == tg$TG)
