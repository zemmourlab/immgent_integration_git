...
# so_orig = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250316.Rds")
# so_orig = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710.Rds")
so_orig = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107.Rds")
so = so_orig
...
**1. Add single_cell_qc.csv and doublet_tcr.csv**
```{r}
...
scqc$doublet_tcr = F
doublet_tcr = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/export_data/doublet_tcr.csv",header = T, sep = ",")[,1]
scqc$doublet_tcr[rownames(scqc) %in% doublet_tcr] = T
...
add annotation_table_history_20251218.csv (from cluster_adjustments.Rmd)
```{r}
# so_orig = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710.Rds")
# so = so_orig
# so$annotation_level1 = NULL
...
annotl1 = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/annotation_table_20251218.csv",header = T, sep = ",", row.names = 1)
all(rownames(annotl1) == colnames(so))
colnames(annotl1)
so$annotation_level1 = annotl1[colnames(so),"level1"]
so$annotation_level2 = annotl1[colnames(so),"level2"]
...
so$annotation_level1 = factor(so$annotation_level1, levels = c("CD8", "CD4", "Treg", "gdT", "CD8aa", "nonconv", "DN", "DP", "thymocyte", "nonT", "remove", "unclear"))
...
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251218.Rds")
...
add annotation_table_history_20260101.csv (from cluster_adjustments.Rmd)
```{r}
# so_orig = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710.Rds")
# so = so_orig
# so$annotation_level1 = NULL
...
annotl1 = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/annotation_table_20260101.csv",header = T, sep = ",", row.names = 1)
all(rownames(annotl1) == colnames(so))
colnames(annotl1)
so$annotation_level1 = annotl1[colnames(so),"level1"]
so$annotation_level2 = annotl1[colnames(so),"level2"]
so$annotation_level2_group = annotl1[colnames(so),"level2.group"]
so$annotation_level2_thymocyte = annotl1[colnames(so),"level2.thymocyte"]

so$annotation_level1 = factor(so$annotation_level1, levels = c("CD8", "CD4", "Treg", "gdT", "CD8aa", "Tz", "DN", "DP", "thymocyte", "nonT", "remove", "unclear"))
...
add annotation_table_history_20260206.csv NEW ANNOTATION (from cluster_adjustments.Rmd)
```{r}
# so_orig = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710.Rds")
# so = so_orig
# so$annotation_level1.v1 = so$annotation_level1
# so$annotation_level2.v1 = so$annotation_level2
# so$annotation_level2_thymocyte.v1 = so$annotation_level2_thymocyte


annotl1 = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/annotation_table_20260206.csv",header = T, sep = ",", row.names = 1)
all(rownames(annotl1) == colnames(so))
colnames(annotl1)
so$annotation_level1 = annotl1[colnames(so),"level1"]
so$annotation_level2 = annotl1[colnames(so),"level2"]
so$annotation_level2_group = annotl1[colnames(so),"level2.group"]
so$annotation_level2_thymocyte = annotl1[colnames(so),"level2.thymocyte"]
so$old_annotation_level2 = annotl1[colnames(so),"level2.v1"]
so$old_annotation_level2_thymocyte = annotl1[colnames(so),"level2.thymocyte.v1"]

so$annotation_level1 = factor(so$annotation_level1, levels = c("CD8", "CD4", "Treg", "gdT", "CD8aa", "Tz", "DN", "DP", "thymocyte", "nonT", "remove", "unclear"))

annotation_table_history = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/annotation_table_history_20260206.csv",header = T, sep = ",")
annotation_level2_order = annotation_table_history %>% arrange(order) %>% pull(level2) %>% as.character()
...
so@meta.data %>% count(annotation_level1, annotation_level2, annotation_level2_group)
so@meta.data %>% count(annotation_level2, old_annotation_level2)

saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206.Rds")
...
m = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/export_data/Sample_metadata_david_20260107_v11.csv",header = T, sep = ",")
...
```{r}
tmp = base::merge(so@meta.data, m, by = "IGTHT", all.x = T, all.y = F)
colnames(tmp)
rownames(tmp) = tmp$cellID
tmp = tmp[colnames(so),]
all(rownames(tmp) == colnames(so))
...
**8. Change organ_simplified**
```{r}
so$organ_simplified0 = so$organ_simplified
so$organ_simplified[so$organ == "colonic epithelium"] = "colon epi"
so$organ_simplified[so$organ == "lamina propria of mucosa of colon"] = "colon LP"
so$organ_simplified[so$organ %in% c("epithelium of small intestine","duodenal epithelium","jejunal epithelium","ileal epithelium")] = "small intestine epi"
so$organ_simplified[so$organ %in% c("lamina propria of small intestine","ileum lamina propria")] = "small intestine LP"
so$organ_simplified = factor(so$organ_simplified, levels = c("blood","spleen","LN","SLO","bone marrow","thymus",
...
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251128.Rds") ##  Treg_cl7--> Treg_cl1, nonconv_cl17-->DP_cl17, nonconv_cl55-->DP_cl55 
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251218.Rds") ##  DP_cl14 --> unclear and metadata v10, doublet_tcr
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251231.Rds") ## nonconv renames ZBTB16
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260101.Rds") ## nonconv renames Tz
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107.Rds") ## metadatav11
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206.Rds")
```

**11. Save clean version + add mde_incremental**
...
so$annotation_level1 = factor(so$annotation_level1, levels = c("CD8", "CD4", "Treg", "gdT", "CD8aa", "Tz", "DN", "DP", "thymocyte"))
...
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710_clean.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251128_clean.Rds") 
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251218_clean.Rds") 
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107_clean.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206_clean.Rds")

```


**12.Save version with CITEseq only**
```{r}
DefaultAssay(so) = "ADT"
so[['RNA']] = NULL

# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250513_clean_ADTonly.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710_clean_ADTonly.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251128_clean_ADTonly.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251218_clean_ADTonly.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20251231_clean_ADTonly.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260101_clean_ADTonly.Rds")
# saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107_clean_ADTonly.Rds")
saveRDS(so,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206_clean_ADTonly.Rds")
...
**12. Split by lineage**
```{r}
prefix = "igt1_96_withtotalvi20260206_clean.Rds"
so = readRDS(prefix)

cd4 = so[,so$annotation_level1 == "CD4"]
cd4
saveRDS(cd4,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD4_%s", prefix))

cd8 = so[,so$annotation_level1 == "CD8"]
saveRDS(cd8,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD8_%s", prefix))

treg = so[,so$annotation_level1 == "Treg"]
saveRDS(treg,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/Treg_%s", prefix))

gdt = so[,so$annotation_level1 == "gdT"]
saveRDS(gdt,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/gdT_%s", prefix))

cd8aa = so[,so$annotation_level1 == "CD8aa"]
saveRDS(cd8aa,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/CD8aa_%s", prefix))

dn = so[,so$annotation_level1 == "DN"]
saveRDS(dn,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/DN_%s", prefix))

dp = so[,so$annotation_level1 == "DP"]
saveRDS(dp,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/DP_%s", prefix))

# nonconv = so[,so$annotation_level1 == "nonconv"]
# saveRDS(nonconv,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/nonconv_%s", prefix))

tz = so[,so$annotation_level1 == "Tz"]
saveRDS(tz,sprintf("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/Tz_%s", prefix))
...
####
m$organ_simplified0 = m$organ_simplified
m$organ_simplified[m$organ == "colonic epithelium"] = "colon epi"
m$organ_simplified[m$organ == "lamina propria of mucosa of colon"] = "colon LP"
m$organ_simplified[m$organ %in% c("epithelium of small intestine","duodenal epithelium","jejunal epithelium","ileal epithelium")] = "small intestine epi"
m$organ_simplified[m$organ %in% c("lamina propria of small intestine","ileum lamina propria")] = "small intestine LP"
m$organ_simplified = factor(m$organ_simplified, levels = c("blood","spleen","LN","SLO","bone marrow","thymus",
                                                                   "lung", "liver", "peritoneal cavity",
                                                                   "colon epi","small intestine epi","colon LP","small intestine LP","skin",
                                                                   "mammary gland","uterus", "placenta", "prostate","submandibular gland","kidney", "pancreas","CNS","synovial fluid"))
...
m$organ_simplified = factor(m$organ_simplified, levels = c("blood","spleen","LN","SLO","bone marrow","thymus",
                                                                   ...
                                                                   "colon epi","small intestine epi","colon LP","small intestine LP","skin",
                                                                   "mammary gland","uterus", "placenta", "prostate","submandibular gland","kidney", "pancreas","CNS","synovial fluid"))
m %>% select(IGTHT,organ_simplified) %>% write.table("~/Desktop/org_meta.txt", quote = F, row.names = F, sep = "\t")
```
...
sub = so_orig@meta.data %>% group_by(IGTHT) %>% dplyr::slice_sample(n = 50) %>% ungroup() %>% pull(cellID)
so_sub = so_orig[,sub]
saveRDS(so_sub,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206_clean_50CellsPerIGTHT.Rds")

so_sub = so_orig[,so_orig$IGT %in% c("IGT36", "IGT38", "IGT40")]
saveRDS(so_sub,"/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206_clean_IGT363840.Rds")
