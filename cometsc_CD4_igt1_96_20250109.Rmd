...
**Exploring the data with subset of cells for finding the gates: CD62L-**
...
for (i in 1:nrow(pops)) {
    ...
    # j = toplots[2]
    pdf(sprintf("gating_strategy_%s.pdf", pop), 6, 6, useDingbats = F)
    for (j in toplots) {
        print(j)
...
saveRDS(gate_list, file = "GatedCells.Rds")

pdf("gated_cells_inMDE.pdf", 15, 15, useDingbats = F)
for (i in 1:nrow(pops)) {
    pop = rownames(pops)[i]
    print(pop)
    pop_split = strsplit(pop, split = ",") %>% unlist() #if interested population is a mixture of several clusters
    ...
    print(p)
    p = MyDimPlotHighlight(seurat_object = so, umap_to_plot = "mde_incremental", cells_to_highlight = colnames(so)[which(so@meta.data[,sprintf("is_gated_%s", pop)])], highlight_column = "annotation_level2", pixels = c(512, 512), mycols = mypal_level2, title = sprintf("gated %s", pop))
    print(p)
}
