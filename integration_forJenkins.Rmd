...
sinteractive --time=5:00:00 --account=pi-zemmour --partition=gpu --gres=gpu:1 --mem=96GB
...
so$timepoint = factor(so$timepoint, levels = c("D6","D7","D21","D60","D90"))
...
so = so[,!so$sample_name %in% c("CD8.P14.D7.rep2", "CD8.P14.D21.rep2")]
so$sample_name = factor(so$sample_name, levels = c("CD4.tetp.D6", "CD4.tetp.D21", "CD4.tetp.D60", "CD8.P14.D6", "CD8.P14.D7.rep1", "CD8.P14.D21.rep1", "CD8.P14.D60", "CD8.P14.D90"))
...
#use_python("/project/jfkfloor2/zemmourlab/david/envs/scvi/bin/python", required = TRUE) if error in loading np and get the following question: Would you like to create a default Python environment for the %s package?
...
see github immgent_integration_git for code: run_scvi.py: 4 options
--batchkey=dataset
--confoundings='timepoint'
--batchkey=dataset --confoundings='cell_type': dataset and celltype are the same (CD4 v CD8), no difference with --batchkey=dataset only
--batchkey=dataset --confoundings='timepoint,cell_type': dataset and celltype are the same (CD4 v CD8), no difference with --batchkey=dataset only


```{bash}
cd /project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/integration_forJenkins/
SCRIPT_DIR=/project/jfkfloor2/zemmourlab/david/immgent/immgent_integration_git
sbatch $SCRIPT_DIR/run_scvi_wrapper_20240518_jenkins.sh #run the following scripts: 
...
Add new scvi dims in R
```{r}
setwd("/project/jfkfloor2/zemmourlab/david/immgent/analysis/integration/integration_forJenkins/")
so = readRDS("Cd4Cd8_so_norep.Rds")
...
so = AddSCVILatent(so, latent_file = "RmDataset/latent_space.csv", prefix = "scvi_rmdataset")
so = AddSCVILatent(so, latent_file = "RmDatasetTimepoint/latent_space.csv", prefix = "scvi_rmdatasettime")
so = AddSCVILatent(so, latent_file = "RmDatasetCelltype/latent_space.csv", prefix = "scvi_rmdatasetcelltype")
so = AddSCVILatent(so, latent_file = "RmDatasetTimepointCelltype/latent_space.csv", prefix = "scvi_rmdatasettimecelltype")
so = AddSCVILatent(so, latent_file = "SCVI/latent_space.csv", prefix = "scvi")

saveRDS(so, file = "Cd4Cd8_so_norep_withSCVI_20250518.Rds")

#For data with the 2 replicates:
so = readRDS("Cd4Cd8_so.Rds")
so = AddSCVILatent(so, latent_file = "SCVI2/latent_space.csv", prefix = "scvi")
so = AddSCVILatent(so, latent_file = "Cd4Cd8/Cd4Cd8_X_SCVI.csv", prefix = "scvi_rmdataset")

saveRDS(so, file = "Cd4Cd8_so_withSCVI_20250518.Rds")
...
# 
prefix = "RmDataset"
umap_to_plot = "umap_scvi_rmdataset"
...
prefix = "RmDatasetTimepoint"
umap_to_plot = "umap_scvi_rmdatasettime"
pdf(sprintf("%s_UMAP.pdf", prefix), 20, 20, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
...
prefix = "RmDatasetCelltype"
umap_to_plot = "umap_scvi_rmdatasetcelltype"
pdf(sprintf("%s_UMAP.pdf", prefix), 20, 20, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
...
prefix = "RmDatasetTimepointCelltype"
umap_to_plot = "umap_scvi_rmdatasettimecelltype"
pdf(sprintf("%s_UMAP.pdf", prefix), 20, 20, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
...
prefix = "SCVI"
umap_to_plot = "umap_scvi"
pdf(sprintf("SCVI2/%s_UMAP.pdf", prefix), 20, 20, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
        color_by = "dataset", 
...
prefix = "RmDataset"
umap_to_plot = "umap_scvi_rmdataset"
pdf(sprintf("Cd4Cd8/%s_UMAP.pdf", prefix), 20, 20, useDingbats = F)
MyPlots(seurat_object = so, 
        dim1 = so[[umap_to_plot]]@cell.embeddings[,1], 
        dim2 = so[[umap_to_plot]]@cell.embeddings[,2], 
        color_by = "dataset", 
