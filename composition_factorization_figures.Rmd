...
```{r}
output_fig_dir = '/Users/david/Library/CloudStorage/GoogleDrive-zemmour@uchicago.edu/My Drive/ImmgenT/20250109_freeze/paper_cosmology/figures/bits'
```
...
**Power law / long tails of sample distribution**
```{r}
mdata = mdata_s

df = mdata %>% select(IGTHT, ends_with("prop"))
...
df_long$population = gsub("\\.prop","", df_long$population )
df_long$level1 = sub("\\_.*","", df_long$population )

library(ggridges)

ggplot(
  df_long %>% filter(prop > 0),        # get rid of exact zeros so log‐scale doesn’t explode
  aes(x = prop, y = population, fill = population)
) +
  geom_density_ridges(alpha = 0.6, scale = 1.2, rel_min_height = 0.01) +
  facet_wrap(~ level1, scales = "free_y") +
  ...
  labs(
    x     = "Proportions",
    y     = NULL,
    title = "Ridgeline Density of Proportions by Level1"
  )



ggplot(df_long, aes(x = prop, color = population)) +
    geom_density(size = 1) +
    ...
    labs(
        ...
        y     = "Density",
        color = "Population",
        title = "Density of Proportions by Level 1"
    ) +
    theme(
        strip.text = element_text(face = "bold"),
        axis.text.x = element_text(angle = 45, hjust = 1)
...
CD8 in I32H7_colonLP_CD45p_mouse0103

I21H8_smallintestineLP_allT_mouse0035
...
pdf(sprintf("%s/barplot_example_sample_sl1.pdf",output_fig_dir), width = 15, 5,useDingbats = F)
...
pdf(sprintf("%s/%s_CD8cl10_IGTUMAP.pdf",output_fig_dir,s), 5,5, useDingbats = F)
...
pdf(sprintf("%s/AlluvialPlot_IEL_tissue.pdf",output_fig_dir), 25, 8, useDingbats = F) #all cells
...
pdf(sprintf("%s/Rorc_clusters_in_topsamples.pdf",output_fig_dir), 10, 8, useDingbats = F) #all cells
...
gs = c("Rorc", "Ahrr")
pdf(sprintf("%s/Barplot_Rorc_Ahr.pdf",output_fig_dir), 12, 7, useDingbats = F)
for (g in gs) {
    print(g)
    gene_to_plot = g
    data = data.frame(
...
# Create the dot plot
pdf(sprintf("%s/cluster_factor_geom_tile.pdf",output_fig_dir), 5, 8, useDingbats = F)
...
pdf(sprintf("%s/cluster_factor_heatmap_viz1.pdf",output_fig_dir), 10,20, useDingbats = F)
tmp3 = F1
...
pdf(sprintf("%s/cluster_factor_heatmap_viz2_to match sample heatmap.pdf",output_fig_dir), 15,4, useDingbats = F)
df_long = melt(F1)
...
gs = c("F2", "F16")
pdf(sprintf("%s/Barplot_F2_F16.pdf",output_fig_dir), 12, 7, useDingbats = F)
for (g in gs) {
    print(g)
    gene_to_plot = g
...
pdf(sprintf("%s/sample_factor_distrib.pdf",output_fig_dir), 5,5, useDingbats = F)
...
pdf(sprintf("%s/Sample_factor_geom_tile.pdf",output_fig_dir), 10, 10, useDingbats = F)
