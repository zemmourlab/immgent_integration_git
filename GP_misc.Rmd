...
names(signaturelist)[grepl("TFH|Tfh", names(signaturelist))]
...
mdata_orig %>% filter(organ_simplified %in% c("placenta") & infection_agent != "SFB" & condition_broad == "healthy") %>% count(sample_code, infection_agent, condition_broad)
mdata_orig %>% filter(organ_simplified %in% c("uterus") & infection_agent != "SFB" ) %>% count(sample_code, infection_agent, condition_broad)

pdf("/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/misc_plots/Placenta.pdf", 10, 10, useDingbats = F)
mdata_orig %>% filter(organ_simplified %in% c("placenta") & infection_agent != "SFB" & condition_broad == "healthy") %>% count(sample_code, infection_agent, condition_broad)
group1 = mdata_orig %>% filter(organ_simplified %in% c("placenta") & condition_broad == "healthy") %>% pull(cellID) #& infection_agent != "SFB" 
mdata_orig %>% filter(!(organ_simplified %in% c("placenta")) & infection_agent != "SFB" & condition_broad == "healthy") %>% count(sample_code, infection_agent, condition_broad)
group2 = mdata_orig %>% filter(!(organ_simplified %in% c("placenta")) & condition_broad == "healthy") %>% pull(cellID) # & infection_agent != "SFB"
results = FlashierDGE(F1, L1, group1, group2, title_plot = "placenta_vs_all other organs, healthy")
results$p1
results$p2
write.table(results$diff_genes, "/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/misc_plots/placenta_vs_otherorgans_healthy_DGE.txt", sep = "\t")
write.table(results$diff_genes, "/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/misc_plots/placenta_vs_otherorgans_healthy_DiffFactors.txt", sep = "\t")

group1 = mdata_orig %>% filter(organ_simplified %in% c("placenta") & infection_agent != "SFB" & condition_broad == "healthy") %>% pull(cellID)
group2 = mdata_orig %>% filter(organ_simplified == "uterus" & condition_broad == "healthy" & infection_agent != "SFB") %>% pull(cellID)
results = FlashierDGE(F1, L1, group1, group2, title_plot = "placenta_vs_uterus, healthy")
results$p1
results$p2
write.table(results$diff_genes, "/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/misc_plots/placenta_vs_uterus_healthy_DGE.txt", sep = "\t")
write.table(results$diff_genes, "/Users/david/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/misc_plots/placenta_vs_uterus_healthy_DiffFactors.txt", sep = "\t")
dev.off()


mdata_orig %>% filter(IGT %in% c("IGT37", "IGT90"), organ_simplified %in% c("placenta"), condition_broad == "healthy") %>% count(sample_code, infection_agent, condition_broad, organ_simplified)
mdata_orig %>% filter(IGT %in% c("IGT37", "IGT90"), !(organ_simplified %in% c("placenta")), condition_broad == "healthy") %>% count(sample_code, infection_agent, condition_broad, organ_simplified)
group1 = mdata_orig %>% filter(IGT %in% c("IGT37", "IGT90"), organ_simplified %in% c("placenta"), condition_broad == "healthy") %>% pull(cellID)
group1 = mdata_orig %>% filter(IGT %in% c("IGT37", "IGT90"), !(organ_simplified %in% c("placenta")), condition_broad == "healthy") %>% pull(cellID)
results = FlashierDGE(F1, L1, group1, group2, title_plot = "placenta_vs_spleengut_IGT3790, healthy")
results$p1
results$p2
...
```{r}
F1[,30][F1[,30] > 0.1] %>% sort(decreasing = T) %>% write.table("~/Desktop/GP30_up.txt", sep = "\t", quote = F, col.names = F, row.names = T)
F1[,30][F1[,30] < -0.1] %>% sort(decreasing = F) %>% write.table("~/Desktop/GP30_down.txt", sep = "\t", quote = F, col.names = F, row.names = T)
