...
**Create a new environment in Midway3 for module analysis: /project/zemmour/david/envs/scvi_20240315**
...
#Compute local autocorrelation in the TOTALVI latent space
...
plt.savefig("Treg_modules_heatmap_2000genes.pdf", format='pdf', bbox_inches='tight')
plt.close() 

results = hs.results.join(hs.modules) #merges both pd df
results.to_csv('modules_extended_2000genes.csv', sep=',', header=True, index=True)
...
module_scores.head()
module_scores.to_csv('modules_singlecellscores_2000genes.csv', sep=',', header=True, index=True)
...
with plt.rc_context({'savefig.dpi': 300}):
    # You can adjust the figure size and other properties within the context manager
    sc.pl.umap(adata, color=module_cols, frameon=False, vmin=-1, vmax=1, save='_UMAP_plot.pdf')
...
```{r}
module_scores = read.csv("~/google_drive/ImmgenT/analysis/data_integration/IGT1_56/Treg/modules_singlecellscores_2000genes.csv", row.names = 1, header = T, sep = ",")

module_genes = read.csv("~/google_drive/ImmgenT/analysis/data_integration/IGT1_56/Treg/modules_extended_2000genes.csv", header = T, sep = ",")
...
FeaturePlot(so, features = "Areg", raster = F, pt.size = 2, order = T)
FeaturePlot(so, features = "Mki67", raster = F, pt.size = 2, order = T)
FeaturePlot(so, features = "Tbx21", raster = F, pt.size = 2, order = T)
FeaturePlot(so, features = "Nr4a1", raster = F, pt.size = 2, order = T)
FeaturePlot(so, features = "Egr3", raster = F, pt.size = 2, order = T)
...
on count data
in R
- could do in each batch for example then goal would be to harmonize the topics?
- topic on normalized data from SCVI
...
**Method 3: SCVI Amortized LDA**
but doesn't take into account the batch correction
NMF and LDA are the 2 most popular calculation of topics. LDA requires count matrix.
