...
so = so_orig[,so_orig@meta.data %>% filter(annotation_level1 == "Treg" & organ_simplified != "thymus") %>% row.names()]
# so_orig$annotation_level2[so_orig$annotation_level2 == "Treg_cl7"] = "Treg_cl1"
# so$annotation_level2 = factor(so$annotation_level2, levels = c("Treg_cl1", "Treg_cl3", "Treg_cl6", "Treg_cl2","Treg_cl8","Treg_cl4", "Treg_prolif", "Treg_miniverse"))
...
dev.off()
    ...
    p = MyDimPlotHighlight(seurat_object = so, umap_to_plot = "mde_incremental", cells_to_highlight = colnames(so)[which(so$condition_broad == "healthy" & so$organ_simplified %in% "blood")], highlight_column_name = "annotation_level2", pixels = c(512, 512), mycols = mypal_level2, title = "blood", highlight_size = 3, highlight_alpha = 1, print_plot1 = T, print_plot2 = T, labelclusters = T)
    print(p)
