...
dir_with_file = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/abT_DN/20250109_abT_DN"
...
so$level2_new[so$annotation_level2_parent1 == "proliferating"] = "abT_DN_prolif"
so$level2_new[so$annotation_level2_parent1 == "miniverse"] = "abT_DN_miniverse"
...
so$is_ref = "other"
so$is_ref[so$annotation_level1 == "abT_DN" & so$annotation_level1_20241216 == "abT_DN"] = "ref"
so$is_ref[so$annotation_level1 == "abT_DN" & so$annotation_level1_20241216 != "abT_DN"] = "query"
...
pdf(sprintf("%s/MDE_annotation_20250109_TransferLabels_Reclustering_to_see.pdf", "20250109_abT_DN"), 20, 20, useDingbats = F)
...
so$level2_new[so$Cluster_totalvi_20241008_rmIGTsample_Res0.75 %in% "14"] = "abT_DN_cl57"
so$level2_new[so$Cluster_totalvi_20241008_rmIGTsample_Res0.75 %in% "8"] = "abT_DN_cl58"
so$level2_new[so$annotation_level2_parent1  %in% c("proliferating")] = "abT_DN_prolif"
...
so$annotation_level2_parent1[!so$level2_new %in% c("abT_DN_prolif", "abT_DN_miniverse", "not_CD4CD8_cl48", "abT_DN_prolif")] = "other"
table(so$annotation_level2_parent1, so$level2_new)

pdf(sprintf("%s/MDE_annotation_20250109_level2.pdf", "20250109_abT_DN"), 20, 20, useDingbats = F)
DimPlot(so, reduction = "mde_incremental", group.by = "annotation_level1", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal_level1)
DimPlot(so, reduction = "mde_incremental", group.by = "annotation_level1_20241216", raster = T, raster.dpi = c(512,512), label = T) + scale_color_manual(values = mypal)
...
if (all(colnames(so_orig)[so_orig$annotation_level1 == "abT_DN"] == colnames(so))) {
    print("True")
    so_orig$annotation_level2[so_orig$annotation_level1 == "abT_DN"] = so$level2_new[colnames(so_orig)[so_orig$annotation_level1 == "abT_DN"]]
    so_orig$annotation_level2_parent1[so_orig$annotation_level1 == "abT_DN"] = so$annotation_level2_parent1[colnames(so_orig)[so_orig$annotation_level1 == "abT_DN"]]
}

table(so_orig$annotation_level2_parent1[so_orig$annotation_level1 == "abT_DN"], so_orig$annotation_level2[so_orig$annotation_level1 == "abT_DN"])
table(so_orig$annotation_level2[so_orig$annotation_level1 == "abT_DN"], so_orig$annotation_level2_20241216[so_orig$annotation_level1 == "abT_DN"])
...
write.table(x = annot, file = "../annotation_table_20250109_withlevel2CD8AB_CD4_gdT_nonconv_CD8aa_DN.csv", row.names = F, quote = F, sep = ",")
...
setwd("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/DN")
so = so_orig[,so_orig$annotation_level1 == "DN"]
...
MyDimPlotHighlight(so_orig, umap_to_plot = "mde2_totalvi_20241006", cells_to_highlight = names(which(so_orig$annotation_level1 == "DN")), highlight_column_name ="annotation_level2", highlight_size = 0.5, highlight_alpha = 0.5, pixels = c(512, 512), mycols = mypal_level2, print_plot1 = T, print_plot2 = T, labelclusters = T )
...
# saveRDS(so, file = "igt1_96_DN_20250109.Rds")
saveRDS(so, file = "igt1_96_DN_20250212.Rds")

# so$annotation_level2 = as.character(so$annotation_level2)
# so$annotation_level2[so$annotation_level2 == "DNcl29"] = "DN_cl29"
# so$annotation_level2 = factor(so$annotation_level2)
