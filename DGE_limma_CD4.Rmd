...
```{r}
so = readRDS("igt1_96_CD4_20241113.Rds")
so
so_orig = so
...
# write.table(sampled_cells, file = sprintf("%s/%s/IGTHT_annotationlevel2_sampledcells.csv", prefix, prefix2), row.names = F, col.names = F, sep = ",", quote = F)
...
Each cl vs all the others
```{r}
fit = readRDS("igt1_96_CD4_20241113_sampled_fit_onannotation_level2.IGTHT.Rds")
design = fit$design

metadata = data.frame(so@meta.data[,c("annotation_level2","IGTHT")])
