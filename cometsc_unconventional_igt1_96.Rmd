---
title: "cometsc_unconventional"
output: html_document
date: "2024-10-23"
---

Continuation from clustering_unconventional_igt_1_96.Rmd

```{bash}
sinteractive --exclusive --time=12:00:00 --account=pi-zemmour --partition=jfkfloor2 --qos=jfkfloor2 --nodes=1 
sinteractive --time=3:00:00 --account=pi-zemmour --partition=caslake --nodes=1 --mem=96GB
...
Prefix
```{r}
prefix = "cometsc_20241023"
reduc = "mde_totalvi_20241014_unconventional_rmIGTsample"
clusters = "unconventional_clusters"
```
...
Remove IGT90: unconventional5.1/2 not the panel
remove protein with less than 1000 rowSums, except TCRVA11.1-TCRVA11.2
--> cometsc_20240607/totalvi_igt1_56_allgenes_unconventional_20240529_so_withclusters_filteredforCometsc.Rds
```{r}
...
saveRDS(so, sprintf("%s/igt1_96_unconventional_filteredforCometsc.Rds", prefix))

```

Heatmap Clusters ADT
```{r}
so = readRDS(sprintf("%s/igt1_96_unconventional_filteredforCometsc.Rds", prefix))

so = NormalizeData(so, assay = "ADT", normalization.method = "CLR")
adt = so[["ADT"]]$data
tmp = as.data.frame(t(as.matrix(adt)))
tmp$unconventional_clusters = so$unconventional_clusters
tmp2 = melt(tmp)
tmp2 = tmp2 %>% group_by(unconventional_clusters, variable) %>% summarise(mean(value))
tmp2 = dcast(tmp2, variable ~ unconventional_clusters)
rownames(tmp2) = tmp2[,1]
tmp2 = tmp2[,-1]
#tmp2 = apply(tmp2,1, as.numeric)
pdf(sprintf("%s/heatmap_ADT_clusters.pdf", prefix), 10,15)
pheatmap(log10(tmp2+1), cluster_rows = F, cluster_cols = F, main = "Mean CLR ADT per unconventional_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = T, main = "Mean CLR ADT per unconventional_clusters")
pheatmap(log10(tmp2+1), cluster_rows = T, cluster_cols = F, main = "Mean CLR ADT per unconventional_clusters")
dev.off()
...
# Sample cells so that no more than 5000 cells per cluster
sample_cells = so@meta.data %>%
  group_by(unconventional_clusters) %>%
  sample_n(size = min(1000, n()), replace = FALSE) %>%
    pull(colnames)

so = so[,sample_cells]
table(so$unconventional_clusters)
...
module load hdf5/1.12.0 #for BPCells but also others
module load openblas/0.3.13
source activate /project/zemmour/david/envs/cometsc #torch works but scvi not, jax error
module load openblas/0.3.13 #load again or error
```

--> 20241023_k2 folders
--> 20241023_k2_log folders
```{bash}
cd /project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/unconventional/cometsc_20241023

Comet input_data_log.txt input_umap.txt input_clusters.txt 20241023_k2_log -C 36 -K 2

Comet input_data.txt input_umap.txt input_clusters.txt 20241023_k2 -C 36 -K 2 # -C is for cores, -K is the number of gene combinations for two does single and 2 markers -->
...
prefix3 = "markers_unconventionalclusters"
```
...
so = NormalizeData(so, assay = "ADT", normalization.method = "LogNormalize")
adt = as.matrix(so[["ADT"]]$data)
markers = c("CD62L", "KLRG1", "unconventional9B", "FR4", "SCA1", "NEUROPILIN1.CD304", "CD69", "unconventional5RB")
adt = adt[markers,]
tmp = melt(adt)
...
# pdf("markers_unconventionalclusters_20240612_gatingplots.pdf", 5, 5, useDingbats = F)
# pdf("markers_unconventionalclusters_20240612_gatingplots_organ.pdf", 15, 15, useDingbats = F)
...
#cl3 no pregating, just on unconventional9B pos
cl = "cl3"
feature1 = "CD62L"
feature2 = "unconventional9B"
th1 = 5
th2 = 4.5
so@meta.data[,sprintf("is_%s", cl)] = so$Cluster_totalvi20240525rmigtsample_Res0.5 %in%  cl
# FeatureScatter(so,slot = "data",feature1 = feature1, feature2 = feature2, group.by = sprintf("is_%s", cl) ) + scale_color_manual(values = c("grey", "red")) + geom_vline(aes(xintercept = th1), linetype="dashed", color = "brown") + geom_vline(aes(xintercept = th1.2), linetype="dashed", color = "brown") + geom_hline(aes(yintercept = th2), linetype="dashed", color = "brown") + ggtitle(sprintf("is_%s", cl)) 
...
for (i in unique(gates$organ_simplified)){
    ...
    # write.table(stats, file = make.names(sprintf("markers_unconventionalclusters_20240612_statsSeSp_%s.tsv",i)), quote = F, row.names = T, col.names = T, sep = "\t")
