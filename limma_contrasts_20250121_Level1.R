...
metadata = data.frame(so@meta.data[,c("annotation_level1","annotation_level2","annotation_level2_group", "annotation_level2.IGTHT", "IGTHT","organ_simplified","condition_broad", "condition_detailed", "sex")])
metadata = metadata %>% unique()
dim(metadata)
rownames(metadata) = metadata$annotation_level2.IGTHT

message("creating constrats") ##EDIT
metadata = metadata[!metadata$annotation_level1 %in% c("nonT", "remove", "unclear"),]
metadata$annotation_level1 = factor(metadata$annotation_level1, levels = levels(metadata$annotation_level1)[levels(metadata$annotation_level1) %in% metadata$annotation_level1])

contrasts = c()
namescontrasts = c()
for (cl in levels(metadata$annotation_level1)) {
    print(cl)
    group1_filter <- expr(annotation_level1 == cl) ##EDIT
    group2_filter <- expr(!(annotation_level1 %in% cl) ) #& level2_parent1 == "resting" ##EDIT
    groups = GetGroups(metadata, group1_filter,group2_filter, "annotation_level2.IGTHT")
    nmecontrast = sprintf("%s_vs_All", cl) ##EDIT
...
for (i in colnames(cont.matrix)) {
    ...
    tt_list[[i]]$SYMBOL = rownames(tt_list[[i]])# tt_list[[i]]$SYMBOL = gene_symbol
}
