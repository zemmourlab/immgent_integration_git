...
set.seed(1)
library(uwot)
df = readRDS("annotation_level2_PropPerSample_withMetadata.Rds")
...
umap_df %>% filter(clusters == "15") %>% pull(sample_name_new)

umap_df %>% filter(clusters == "12") %>% pull(sample_name_new)
umap_df %>% filter(clusters == "6") %>% pull(sample_name_new)
umap_df %>% filter(clusters == "12") %>% pull(sample_name_new)
...
ggplot(umap_df, aes(x = UMAP_1, y = UMAP_2, color = CD8AB_cl10.prop, size = CD8AB_cl10.prop)) +
  geom_point(alpha = 0.8) +
  scale_color_gradient(low = "blue", high = "red") +
  theme_minimal() +
  labs(title = "UMAP Visualization", x = "UMAP 1", y = "UMAP 2")

ggplot(umap_df, aes(x = UMAP_1, y = UMAP_2, color = CD8AB_cl11.prop, size = CD8AB_cl11.prop)) +
  geom_point(alpha = 0.8) +
  scale_color_gradient(low = "blue", high = "red") +
  theme_minimal() +
  labs(title = "UMAP Visualization", x = "UMAP 1", y = "UMAP 2")

ggplot(umap_df, aes(x = UMAP_1, y = UMAP_2, color = CD8AB_cl12.prop, size = CD8AB_cl12.prop)) +
  geom_point(alpha = 0.8) +
  scale_color_gradient(low = "blue", high = "red") +
  theme_minimal() +
  labs(title = "UMAP Visualization", x = "UMAP 1", y = "UMAP 2")
...
saveRDS(umap_df, file = "annotation_level2_UMAP_samples.Rds")
...
umap_df_melt = melt(umap_df[,grepl("ncells|immgent_IGT_10xlane_id", colnames(umap_df)) ])

umap_df_melt2 = umap_df_melt %>% group_by()
...
pdf("cluster_saturation.pdf", 15, 7,useDingbats = F)
ggplot(df) + geom_point(aes(experiment_number, number_of_clusters), size = 3) + 
    ...
    geom_hline(aes(yintercept = 119), linetype="dashed", color = "brown", linewidth = 1) +
