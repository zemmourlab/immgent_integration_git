...
libs = c("Seurat", "ZemmourLib", "ggplot2", "dplyr", "ggrastr", "RColorBrewer", "pals", "scales", "reshape2", "pheatmap", "scattermore") 
...
# prefix = "SampleComp/20250226_2"
# prefix = "SampleComp/20260107"
prefix = "SampleComp/20260206"
...
# so = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250710_clean.Rds") 
...
# so = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260107_clean.Rds") 
so = readRDS("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20260206_clean.Rds")
...
# sample_metadata = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/export_data/Sample_metadata_david_20250628_v10.csv",header = T, sep = ",")
sample_metadata = read.table("/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/export_data/Sample_metadata_david_20260107_v11.csv",header = T, sep = ",")
...
for (i in c("CD4", "CD8", "Treg", "gdT", "CD8aa","Tz", "DN", "DP")) {
    print(i)
    colz = grepl(sprintf("%s\\.",i), colnames(tmp3)) #colz = grepl(sprintf("%s_",i), colnames(tmp3))
...
for(i in c("CD4", "CD8", "Treg", "gdT", "CD8aa","Tz", "DN", "DP")) {
    print(i)
    sub = c(which(grepl(sprintf("%s\\.",i), colnames(m3))), seq(which(colnames(m3) == "sample_name"),ncol(m3)))
...
for (i in c("CD4", "CD8", "Treg", "gdT", "CD8aa","Tz", "DN", "DP")) {
    ...
    colz = grepl(sprintf("%s\\.",i), colnames(tmp3))
...
m2 = m # m2 = merge(m, sample_metadata, by.x = "IGTHT", by.y = "IGTHT",all.x = T, all.y = F, suffixes = c(".x", ""))
...
all(ncells$organ_simplified %in% stat_df_ncells$organ_simplified)
...
for( i in c("CD4", "CD8", "Treg", "gdT", "CD8aa","Tz", "DN", "DP")) {
    print(i)
    sub = c(which(grepl(sprintf("%s\\.|organ_simplified",i), colnames(m3))))
