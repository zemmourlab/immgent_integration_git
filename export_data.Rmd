...
#source activate /project/jfkfloor2/zemmourlab/david/envs/scvi #source activate /project/zemmour/david/envs/scvi_20240315
...
prefix = "igt1_96_20250109"
so_orig = readRDS(file = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250109.Rds")

so_orig = readRDS(file = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/igt1_96_withtotalvi20250513_clean.Rds")
...
Export in-memory diet seurat object (no metadata, no umaps etc)
```{r}
so = so_orig
so@meta.data = so@meta.data[,c("IGTHT", "nCount_RNA", "nFeature_RNA", "nCount_ADT", "nFeature_ADT")]

file_dir = "export_data"
so[["RNA"]]$counts = as(object = so[["RNA"]]$counts, Class = "dgCMatrix")
DefaultAssay(so) = "RNA"
...
saveRDS(object = so, file = sprintf("%s/%s_inmemory_diet_seurat.Rds", file_dir, prefix))

#sod = readRDS(sprintf("export_data/%s_inmemory_diet_seurat.Rds", prefix))
```

Export diet seurat object as BPCells (on-disk instead of in-memory) object and with sketched assay
```{r}
library(BPCells)
...
write_matrix_dir(mat = so[["RNA"]]$counts, dir = sprintf("%s/counts", file_dir)) #sprintf("%s/%s_counts", file_dir, prefix)
counts.mat = open_matrix_dir(dir = sprintf("%s/counts", file_dir)) #sprintf("%s/%s_counts", file_dir, prefix)
...
Export single_cell_qc.csv
```{r}
someta = so@meta.data
colnames(someta)
cellqc = data.frame(IGT_cellID = rownames(someta), someta[,c("nCount_RNA", "nFeature_RNA", "nCount_ADT", "nFeature_ADT", "IGTHT")])
write.table(cellqc, file = "/project/zemmour/david/ImmgenT/analysis/data_integration/IGT1_96/export_data/single_cell_qc.csv", quote = F, sep = ",", row.names = T, col.names = T)
```
...
doesn't work anymore (library issue), just see pytjon code below, instead
```{r}
file_dir = "export_data"
...
**Export seurat object per IGT**
```{r}

igts = unique(so_orig$IGT)

for (i in igts) {
    print(i)
    dir = sprintf("export_data/IGT/20250531/%s",i)
    ensure_directory(dir)
    saveRDS(so_orig[,so_orig$IGT == i], file = sprintf("%s/%s_seurat_immgent_20250531.Rds", dir, i))
}
...
for i in {1..96}; do \
echo IGT$i ; \
#ll ~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/MDE_InEachSample/IGT${i} ;
#ll ~/google_drive/ImmgenT/exp_id/*exp_id*/IGT${i} ; -->
cp ~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/export_data/IGT/20250531/IGT${i}/IGT${i}_seurat_immgent_20250531.Rds ~/google_drive/ImmgenT/exp_id/*exp_id*/IGT${i}/
done 
```
...
#ll ~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/MDE_InEachSample/IGT${i} ;
#ll ~/google_drive/ImmgenT/exp_id/*exp_id*/IGT${i} ;
cp ~/google_drive_uchicago/ImmgenT/analysis/data_integration/IGT1_96/MDE_InEachSample/IGT${i}/* ~/google_drive/ImmgenT/exp_id/*exp_id*/IGT${i}/
done 
...
Don't make h5ad files: weird, doesn't want to save protein names in adata_rna.obsm['protein'].varm..
Make h5mu file!
```{python}
import pandas as pd
...
prefix = "igt1_96_20250109" #totalvi_igt1_96_20241006
...
transposed_matrix_rna = matrix_rna.transpose()
transposed_matrix_protein = matrix_protein.transpose()
...
adata_rna = ad.AnnData(X=transposed_matrix_rna, obs=cell_metadata) #var=pd.DataFrame(index=genes[0])
adata_protein = ad.AnnData(X=transposed_matrix_protein) #var=pd.DataFrame(index=proteins[0])


adata_rna.obs_names = cells[0]
adata_rna.var_names = genes[0]
adata_protein.obs_names = cells[0]
adata_protein.var_names = proteins[0]

# Step 3: Merge RNA and Protein data into one object
# Assume the same cells in the same order for both matrices
adata_rna.obsm['protein'] = adata_protein.X
adata_rna.obsm['protein'].varm = adata_protein.var_names

# Step 4: Save the AnnData object
adata_rna.write(prefix+'.h5ad')


#NB: couldn't write h5ad with protein as panda dataframe
#transposed_matrix_protein = transposed_matrix_protein.toarray()
#protein_df = pd.DataFrame(transposed_matrix_protein)
# protein_df.index = cells[0]
# protein_df.index = protein_df.index.astype(str)
# protein_df.columns = proteins[0]
# protein_df.columns = protein_df.columns.astype(str)
# protein_df = protein_df.astype(str)
# data_protein = ad.AnnData(X=protein_df) #var=pd.DataFrame(index=proteins[0])
# adata_rna.obsm['protein'] = protein_df
...
adata = sc.read_h5ad(prefix+".h5ad")

#model = scvi.model.TOTALVI.load("totalvi_igt1_56_allgenes_Treg_20240306/")
mdata = mu.MuData({"RNA": adata})
#mdata.mod["RNA"].layers["counts"] = mdata.mod["RNA"].X.copy() #doubles the size of the written object at the end, so remove, not necessary
...
#protein_data = protein_data[:,non_zero_columns]
protein_adata = sc.AnnData(X=protein_data)
mdata.mod['protein'] = protein_adata
#mdata.mod["protein"].layers["counts"] = mdata.mod["protein"].X.copy() ##doubles the size of the written object at the end, so remove, not necessary
mdata.mod['protein'].var_names = adata_protein.var_names
mdata.mod['protein'].obs_names = adata.obs_names
mdata.mod['protein'].X = mdata.mod['protein'].X.toarray() #totalvi doesn't want sparse matrix for protein. get index error with scvi.model.TOTALVI
...
#mdata = mu.read(prefix+".h5mu")

```

EXTRA
not useful anymore, already cleaned up upstream
```{python}
#import metadata
