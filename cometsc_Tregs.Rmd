...
Continuation from clustering_Tregs.Rmd
...
#remove IGT1 (CITEseq V1) and remove IGT7,8,9,44,45,46 (no ADT)
so = so_orig[,!so$IGT %in% c("IGT1","IGT7","IGT8","IGT9","IGT44","IGT45","IGT46")]
names(which(colSums(so[["ADT"]]$counts)==0))
...
saveRDS(so, sprintf("%s/totalvi_igt1_56_allgenes_Treg_20240529_so_withclusters_filteredforCometsc.Rds", prefix))
...
vis = so[["umap_totalvi_20240525_rmigtsample"]]@cell.embeddings
write.table(vis, file = sprintf("%s/input_umap.txt",prefix), sep = "\t", row.names = T, col.names = F, quote = F)

clusters = data.frame(cell_id = colnames(so), clusters = so$Cluster_totalvi20240525rmigtsample_Res0.5)
write.table(clusters, file = sprintf("%s/input_clusters.txt",prefix), sep = "\t", row.names = F, col.names = F, quote = F)

gene_list = rownames(so[["ADT"]])
write.table(gene_list, file = sprintf("%s/input_markers.txt",prefix), sep = "\t", row.names = F, col.names = F, quote = F)
```
...
--> 20240609_k2 folders
--> 20240609_k2_log folders
```{bash}
cd cometsc_20240607

Comet input_data.txt input_umap.txt input_clusters.txt 20240609_k2 -C 36 -K 2
Comet input_data_log.txt input_umap.txt input_clusters.txt 20240609_k2_log -C 36 -K 2
...
```{r}
prefix = "cometsc_20240607"
prefix2 = "20240609_k2_logdata"
prefix3 = "markers_Tregclusters_20240612"
```

```{r}
so = readRDS(sprintf("%s/totalvi_igt1_56_allgenes_Treg_20240529_so_withclusters_filteredforCometsc.Rds", prefix))
```
